...
import { canUseDirectorySession, hasDirectoryCapability } from "@/lib/auth/access";
...
import type { ConversationMetrics, DocumentMetrics, WorkflowMetrics } from "@/components/workspace/workspace-types";
...
const WORKSPACE_LIFECYCLE_FILTER_VALUES = ["all", "active", "archived"] as const;
const KNOWLEDGE_BASE_PUBLICATION_FILTER_VALUES = ["all", "published", "draft"] as const;
const MEMBER_ACCOUNT_FILTER_VALUES = ["all", "active", "inactive"] as const;
const MEMBER_RELATIONSHIP_FILTER_VALUES = ["all", "active", "invited", "suspended"] as const;
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 sm:p-6">
        <div className="flex max-h-[calc(100vh-2rem)] w-full max-w-[680px] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl sm:max-h-[calc(100vh-3rem)]">
...
function readAllowedFilterValue(value: string | null, allowedValues: readonly string[], fallbackValue = "all") {
  if (!value) {
...
function readAllowedAdminManagementPanel(
  value: string | null
): Extract<AdminManagementPanel, "workspace-edit" | "knowledge-base-edit" | "user-edit"> | null {
  if (value === "workspace-edit" || value === "knowledge-base-edit" || value === "user-edit") {
    return value;
  }
...
async function loadConversationMetrics(tenantId: string) {
  return await apiRequest<ConversationMetrics>(`/chat/conversations/metrics?tenant_id=${tenantId}`);
}
...
async function setWorkspaceArchiveState(workspaceId: string, tenantId: string, isArchived: boolean) {
  return await apiRequest<Workspace>(`/workspaces/${workspaceId}/lifecycle?tenant_id=${tenantId}`, {
    method: "POST",
    body: JSON.stringify({
...
async function updateWorkspaceResource(
  ...
  return await apiRequest<Workspace>(`/workspaces/${workspaceId}?tenant_id=${tenantId}`, {
    method: "PATCH",
    body: JSON.stringify(payload)
  });
}
...
async function updateKnowledgeBaseResource(
  knowledgeBaseId: string,
  workspaceId: string,
  payload: { name: string; slug: string; description?: string | null; retrieval_profile_id?: string | null }
) {
  return await apiRequest<KnowledgeBase>(`/knowledge-bases/${knowledgeBaseId}?workspace_id=${workspaceId}`, {
    method: "PATCH",
    body: JSON.stringify(payload)
  });
}
...
async function listUsers(filters?: {
  ...
  return await apiRequest<UserDirectoryItem[]>(queryString.length > 0 ? `/users?${queryString}` : "/users");
}
...
async function updateUserMembershipStatus(
  ...
  return await apiRequest<UserDirectoryItem>(`/users/${userId}/memberships/${membershipId}`, {
    method: "PATCH",
    body: JSON.stringify(payload)
  });
}

async function issueUserMembershipInvitation(userId: string, membershipId: string, reason?: string) {
...
async function revokeUserMembershipInvitation(userId: string, membershipId: string, reason?: string) {
  return await apiRequest<UserDirectoryItem>(`/users/${userId}/memberships/${membershipId}/revoke-invitation`, {
    method: "POST",
    body: JSON.stringify({ reason: reason ?? null })
  });
}
...
async function loadAdminDirectory(filters: {
  ...
  const users = await listUsers({
    ...
    membershipStatus: filters.memberRelationshipFilter as "active" | "invited" | "suspended" | "all",
    isActive: filters.memberAccountFilter as "active" | "inactive" | "all",
  });
  const agentGroups = await Promise.all(
    tenants.map(async (tenant) => ({
      tenantId: tenant.id,
      agents: await listAgentDefinitions(tenant.id)
    }))
  );
  ...
  const tenantWorkspaceGroups = await Promise.all(
    tenants.map(async (tenant) => {
      ...
      if (filters.workspaceLifecycleFilter !== "all") {
        searchParams.set("is_archived", filters.workspaceLifecycleFilter === "archived" ? "true" : "false");
      }

      return {
        tenant,
        workspaces: await apiRequest<Workspace[]>(`/workspaces?${searchParams.toString()}`)
      };
    })
  );
  ...
  const knowledgeBaseGroups = await Promise.all(
    workspaces.map(async (workspace) => {
      ...
      if (filters.knowledgeBasePublicationStatusFilter !== "all") {
        searchParams.set("publication_status", filters.knowledgeBasePublicationStatusFilter);
      }

      return {
        workspaceId: workspace.id,
        knowledgeBases: await apiRequest<KnowledgeBase[]>(`/knowledge-bases?${searchParams.toString()}`)
      };
    })
  );
  ...
  const auditEvents = await apiRequest<UserAccessEvent[]>(`/users/audit-events?${auditSearchParams.toString()}`);
...
function getWorkspaceLifecycleClass(isArchived: boolean | undefined) {
  return isArchived ? "border-amber-200 bg-amber-50 text-amber-700" : "border-emerald-200 bg-emerald-50 text-emerald-700";
}

function getKnowledgeBasePublicationClass(publicationStatus: string | undefined) {
...
function getAccessEventLabel(eventType: string, t: (key: string, variables?: Record<string, string>) => string) {
...
function getDefaultGovernanceReason(action:
  | "issue_invitation"
  | "revoke_invitation"
  | "activate_membership"
  | "suspend_membership") {
...
export default function AdminConsolePage() {
  ...
  const hasAdminWriteAccess = hasDirectoryCapability(session, "manage_admin_resources");
  ...
  const [tenantWorkspaceGroups, setTenantWorkspaceGroups] = useState<TenantWorkspaceGroup[]>([]);
  ...
  const [workspaceLifecycleFilter, setWorkspaceLifecycleFilter] = useState<string>("all");
  const [knowledgeBasePublicationStatusFilter, setKnowledgeBasePublicationStatusFilter] = useState<string>("all");
  const [retrievalProfileFilter, setRetrievalProfileFilter] = useState<string>("all");
  const [memberAccountFilter, setMemberAccountFilter] = useState<string>("all");
  const [memberRelationshipFilter, setMemberRelationshipFilter] = useState<string>("all");
  const [auditEventFilter, setAuditEventFilter] = useState<string>("all");
  const [searchQuery, setSearchQuery] = useState("");
  const [statusMessage, setStatusMessage] = useState(() => t("admin.status.loading"));
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [lastRefreshedAt, setLastRefreshedAt] = useState<string | null>(null);
  const { runtimeHealth, isLoadingRuntimeHealth, runtimeHealthErrorMessage } = useRuntimeHealth({
    enabled: Boolean(session)
  });
  const [conversationMetricsByTenantId, setConversationMetricsByTenantId] = useState<Record<string, ConversationMetrics>>({});
  const [documentMetricsByKnowledgeBaseId, setDocumentMetricsByKnowledgeBaseId] = useState<Record<string, DocumentMetrics>>({});
  const [workflowMetricsByTenantId, setWorkflowMetricsByTenantId] = useState<Record<string, WorkflowMetrics>>({});
  const [agentRunMetricsByTenantId, setAgentRunMetricsByTenantId] = useState<Record<string, AgentRunMetricsResponse>>({});
  const [recentAgentRunsByTenantId, setRecentAgentRunsByTenantId] = useState<Record<string, AgentRunResponse[]>>({});
  const [agentExecutionMetricsByTenantId, setAgentExecutionMetricsByTenantId] = useState<Record<string, AgentExecutionMetricsResponse>>({});
  const [recentAgentExecutionsByTenantId, setRecentAgentExecutionsByTenantId] = useState<Record<string, AgentExecutionResponse[]>>({});
  ...
  const [activeWorkspaceActionId, setActiveWorkspaceActionId] = useState<string | null>(null);
  const [activeKnowledgeBaseActionId, setActiveKnowledgeBaseActionId] = useState<string | null>(null);
  const [activeUserActionId, setActiveUserActionId] = useState<string | null>(null);
  const [activeMembershipActionId, setActiveMembershipActionId] = useState<string | null>(null);
  const [revealedInvitationByMembershipId, setRevealedInvitationByMembershipId] = useState<
    Record<string, MembershipInvitationCredential>
  >({});
  const [managementPanel, setManagementPanel] = useState<AdminManagementPanel>(null);
  ...
  const [createKnowledgeBaseWorkspaceId, setCreateKnowledgeBaseWorkspaceId] = useState("");
  ...
  const [createKnowledgeBaseDescription, setCreateKnowledgeBaseDescription] = useState("");
  const [createKnowledgeBaseRetrievalProfileId, setCreateKnowledgeBaseRetrievalProfileId] = useState("");
  const [editingWorkspaceId, setEditingWorkspaceId] = useState<string | null>(null);
  const [pendingWorkspaceFocusId, setPendingWorkspaceFocusId] = useState<string | null>(null);
  ...
  const [pendingKnowledgeBaseFocusId, setPendingKnowledgeBaseFocusId] = useState<string | null>(null);
  const [editKnowledgeBaseWorkspaceId, setEditKnowledgeBaseWorkspaceId] = useState("");
  ...
  const [createUserMembershipStatus, setCreateUserMembershipStatus] = useState<"active" | "invited" | "suspended">("invited");
  const [createUserRole, setCreateUserRole] = useState<"super_admin" | "operator" | "reviewer">("operator");
  const [editingUserId, setEditingUserId] = useState<string | null>(null);
  const [pendingUserFocusId, setPendingUserFocusId] = useState<string | null>(null);
  const [editUserDisplayName, setEditUserDisplayName] = useState("");
  const [editUserEmail, setEditUserEmail] = useState("");
  const [editUserIsActive, setEditUserIsActive] = useState<"active" | "inactive">("active");
  const [editUserRole, setEditUserRole] = useState<"super_admin" | "operator" | "reviewer">("operator");

  function syncCurrentSessionFromDirectoryUser(user: UserDirectoryItem) {
    if (!session?.userId || session.userId !== user.id) {
      return;
    }

    if (!user.is_active || !canUseDirectorySession(user.role, user.memberships)) {
  ...
  useEffect(() => {
    function applyLocationState() {
      ...
      const requestedManagementPanel = readAllowedAdminManagementPanel(searchParams.get("management_panel"));
      const requestedSection = readAllowedAdminSection(searchParams.get("section"));
      const normalizedRequestedSection =
        !showAdvancedAdminSections && requestedSection === "security" ? "access" : requestedSection;
      const requestedWorkspaceId = searchParams.get("workspace_id")?.trim() ?? "";
      const requestedKnowledgeBaseId = searchParams.get("knowledge_base_id")?.trim() ?? "";
      const requestedUserId = searchParams.get("user_id")?.trim() ?? "";
      setAdminSection(
        requestedManagementPanel === "workspace-edit" && requestedWorkspaceId
          ? "directory"
          : requestedManagementPanel === "knowledge-base-edit" && requestedKnowledgeBaseId
          ? "directory"
      ...
      setWorkspaceLifecycleFilter(
        readAllowedFilterValue(searchParams.get("workspace_lifecycle"), WORKSPACE_LIFECYCLE_FILTER_VALUES)
      );
      ...
      setMemberAccountFilter(readAllowedFilterValue(searchParams.get("member_account"), MEMBER_ACCOUNT_FILTER_VALUES));
      setMemberRelationshipFilter(
        readAllowedFilterValue(searchParams.get("member_relationship"), MEMBER_RELATIONSHIP_FILTER_VALUES)
      );
      setAuditEventFilter(readAllowedFilterValue(searchParams.get("audit_event"), AUDIT_EVENT_FILTER_VALUES));
      setPendingWorkspaceFocusId(
        requestedManagementPanel === "workspace-edit" && requestedWorkspaceId ? requestedWorkspaceId : null
      );
      setPendingKnowledgeBaseFocusId(
        requestedManagementPanel === "knowledge-base-edit" && requestedKnowledgeBaseId ? requestedKnowledgeBaseId : null
      );
      setPendingUserFocusId(requestedManagementPanel === "user-edit" && requestedUserId ? requestedUserId : null);
  ...
  useEffect(() => {
    ...
    if (knowledgeBasePublicationStatusFilter !== "all") {
      nextUrl.searchParams.set("knowledge_base_publication", knowledgeBasePublicationStatusFilter);
    } else {
    ...
    if (managementPanel === "workspace-edit" && editingWorkspaceId) {
    ...
    } else if (managementPanel === "knowledge-base-edit" && editingKnowledgeBaseId) {
      ...
      nextUrl.searchParams.set("management_panel", "knowledge-base-edit");
      nextUrl.searchParams.set("knowledge_base_id", pendingKnowledgeBaseFocusId);
      nextUrl.searchParams.delete("workspace_id");
      nextUrl.searchParams.delete("user_id");
  ...
  }, [adminSection, selectedTenantId, workspaceLifecycleFilter, knowledgeBasePublicationStatusFilter, retrievalProfileFilter, memberAccountFilter, memberRelationshipFilter, auditEventFilter, managementPanel, editingWorkspaceId, editingKnowledgeBaseId, editingUserId, pendingWorkspaceFocusId, pendingKnowledgeBaseFocusId, pendingUserFocusId, searchQuery]);

  useEffect(() => {
    if (!pendingWorkspaceFocusId || !hasAdminWriteAccess || workspaces.length === 0) {
      return;
    }

    const targetWorkspace = workspaces.find((workspace) => workspace.id === pendingWorkspaceFocusId);
    if (!targetWorkspace) {
      return;
    }
    ...
    if (managementPanel !== "workspace-edit" || editingWorkspaceId !== targetWorkspace.id) {
  ...
  useEffect(() => {
    if (!pendingKnowledgeBaseFocusId || !hasAdminWriteAccess || knowledgeBases.length === 0) {
      return;
    }
    ...
    if (managementPanel !== "knowledge-base-edit" || editingKnowledgeBaseId !== targetKnowledgeBase.id) {
  ...
  async function refreshAdminDirectory() {
    ...
    try {
      ...
      const [
        ...
        Promise.all(directory.tenants.map(async (tenant) => [tenant.id, await loadConversationMetrics(tenant.id)] as const)),
        Promise.all(directory.knowledgeBases.map(async (knowledgeBase) => [knowledgeBase.id, await loadDocumentMetrics(knowledgeBase.id)] as const)),
        Promise.all(directory.tenants.map(async (tenant) => [tenant.id, await loadWorkflowMetrics(tenant.id)] as const)),
        Promise.all(directory.tenants.map(async (tenant) => [tenant.id, await listAgentRunMetrics(tenant.id)] as const)),
        Promise.all(directory.tenants.map(async (tenant) => [tenant.id, await listAgentRuns(tenant.id, null, 4)] as const)),
        Promise.all(directory.tenants.map(async (tenant) => [tenant.id, await listAgentExecutionMetrics(tenant.id)] as const)),
        Promise.all(directory.tenants.map(async (tenant) => [tenant.id, await listAgentExecutions(tenant.id, null, 4)] as const)),
        loadPlatformGovernanceSnapshot(),
        loadAgentRuntimeGovernance({
      ...
      setConversationMetricsByTenantId(Object.fromEntries(conversationMetricEntries));
      setDocumentMetricsByKnowledgeBaseId(Object.fromEntries(documentMetricEntries));
      ...
      setAgentExecutionMetricsByTenantId(Object.fromEntries(agentExecutionMetricEntries));
      setRecentAgentExecutionsByTenantId(Object.fromEntries(recentAgentExecutionEntries));
      ...
      setErrorMessage(error instanceof Error ? error.message : t("admin.status.failed"));
  ...
  async function handleToggleWorkspaceLifecycle(workspace: Workspace) {
    try {
      ...
      setErrorMessage(null);
      setStatusMessage(
        workspace.is_archived ? t("admin.status.restoreWorkspace") : t("admin.status.archiveWorkspace")
      );
      const nextWorkspace = await setWorkspaceArchiveState(workspace.id, workspace.tenant_id, !workspace.is_archived);
      ...
      setErrorMessage(error instanceof Error ? error.message : t("admin.status.workspaceActionFailed"));
  ...
  async function handleToggleKnowledgeBasePublication(knowledgeBase: KnowledgeBase) {
    const nextPublicationStatus = knowledgeBase.publication_status === "published" ? "draft" : "published";
    ...
    try {
      ...
      setStatusMessage(
        nextKnowledgeBase.publication_status === "published"
          ? t("admin.status.knowledgeBasePublished", { name: nextKnowledgeBase.name })
          : t("admin.status.knowledgeBaseDraft", { name: nextKnowledgeBase.name })
      );
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : t("admin.status.knowledgeBaseActionFailed"));
  ...
  function openCreateWorkspacePanel() {
    ...
    setCreateWorkspaceTenantId(selectedTenantId !== "all" ? selectedTenantId : tenants[0]?.id ?? "");
  ...
  function openCreateUserPanel() {
    ...
    setCreateUserEmail("");
    setCreateUserTenantId(selectedTenantId !== "all" ? selectedTenantId : "none");
  ...
  function openCreateKnowledgeBasePanel() {
    ...
    setCreateKnowledgeBaseWorkspaceId(scopedPrimaryWorkspace?.id ?? filteredWorkspaces[0]?.id ?? "");
  ...
  function openKnowledgeBaseEditPanel(knowledgeBase: KnowledgeBase) {
    ...
    setEditKnowledgeBaseRetrievalProfileId(knowledgeBase.retrieval_profile_id ?? "");
  ...
  async function handleCreateTenant() {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : t("admin.status.tenantCreationFailed"));
  ...
  async function handleCreateWorkspace() {
    ...
    try {
      ...
      await refreshAdminDirectory();
      setStatusMessage(t("admin.status.workspaceCreated", { name: workspace.name }));
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : t("admin.status.workspaceCreationFailed"));
  ...
  async function handleUpdateWorkspace() {
    ...
    if (!editingWorkspaceId || !editWorkspaceTenantId || !name || !slug || isUpdatingResource) {
      return;
    }
    ...
    try {
      ...
      const workspace = await updateWorkspaceResource(editingWorkspaceId, editWorkspaceTenantId, {
        name,
        slug,
      ...
      setManagementPanel(null);
      await refreshAdminDirectory();
      setStatusMessage(t("admin.status.workspaceUpdated", { name: workspace.name }));
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : t("admin.status.workspaceUpdateFailed"));
  ...
  async function handleCreateKnowledgeBase() {
    ...
    const workspace = filteredWorkspaces.find((item) => item.id === workspaceId) ?? workspaces.find((item) => item.id === workspaceId);
    ...
    try {
      ...
      await refreshAdminDirectory();
      setStatusMessage(t("admin.status.knowledgeBaseCreated", { name: knowledgeBase.name }));
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : t("admin.status.knowledgeBaseCreationFailed"));
  ...
  async function handleCreateUser() {
    ...
    try {
      ...
      const user = await createUserDirectoryEntry({
        ...
        ...(createUserTenantId !== "none" ? { tenant_id: createUserTenantId } : {})
      });
      setManagementPanel(null);
      await refreshAdminDirectory();
      setStatusMessage(t("admin.status.memberCreated", { name: user.display_name }));
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : t("admin.status.memberCreationFailed"));
  ...
  async function handleUpdateUser() {
    ...
    try {
      ...
      syncCurrentSessionFromDirectoryUser(user);
      setManagementPanel(null);
      await refreshAdminDirectory();
      setStatusMessage(t("admin.status.memberUpdated", { name: user.display_name }));
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : t("admin.status.memberUpdateFailed"));
  ...
  async function handleAddUserToTenant(user: UserDirectoryItem) {
    ...
    try {
      ...
      syncCurrentSessionFromDirectoryUser(updatedUser);
      await refreshAdminDirectory();
      setStatusMessage(t("admin.status.memberInvitedToTenant", { name: user.display_name }));
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : t("admin.status.memberAddFailed"));
  ...
  async function handleUpdateMembership(
    ...
    try {
      ...
      setErrorMessage(null);
      setStatusMessage(
        membershipStatus === "active" ? t("admin.status.activatingMembership") : t("admin.status.suspendingMembership")
      );
      const updatedUser = await updateUserMembershipStatus(user.id, membership.id, {
        membership_status: membershipStatus,
        reason: getDefaultGovernanceReason(
          membershipStatus === "active" ? "activate_membership" : "suspend_membership"
        )
      });
      syncCurrentSessionFromDirectoryUser(updatedUser);
      await refreshAdminDirectory();
      ...
      setErrorMessage(error instanceof Error ? error.message : t("admin.status.membershipUpdateFailed"));
  ...
  async function handleIssueMembershipInvitation(user: UserDirectoryItem, membership: UserMembership) {
    if (!hasAdminWriteAccess) {
      return;
    }
    ...
    try {
      ...
      await refreshAdminDirectory();
      setStatusMessage(t("admin.status.invitationCodeIssued", { name: user.display_name }));
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : t("admin.status.invitationCodeIssueFailed"));
  ...
  async function handleRevokeMembershipInvitation(user: UserDirectoryItem, membership: UserMembership) {
    if (!hasAdminWriteAccess) {
      return;
    }
    ...
    try {
      ...
      syncCurrentSessionFromDirectoryUser(updatedUser);
      await refreshAdminDirectory();
      setStatusMessage(t("admin.status.invitationCodeRevoked", { name: user.display_name }));
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : t("admin.status.invitationCodeRevokeFailed"));
  ...
  async function handleRemoveMembership(user: UserDirectoryItem, membership: UserMembership) {
    if (!hasAdminWriteAccess) {
      return;
    }
    ...
    try {
      ...
      syncCurrentSessionFromDirectoryUser(updatedUser);
      await refreshAdminDirectory();
      setStatusMessage(t("admin.status.membershipRemoved", { name: user.display_name }));
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : t("admin.status.membershipRemoveFailed"));
  ...
  async function handleToggleUserAccount(user: UserDirectoryItem) {
    ...
    try {
      setActiveUserActionId(user.id);
      setErrorMessage(null);
      setStatusMessage(user.is_active ? t("admin.status.deactivatingMember") : t("admin.status.activatingMemberAccount"));
      ...
      setErrorMessage(error instanceof Error ? error.message : t("admin.status.memberAccountUpdateFailed"));
  ...
  async function handleUpdateKnowledgeBase() {
    ...
    if (!editingKnowledgeBaseId || !editKnowledgeBaseWorkspaceId || !name || !slug || isUpdatingResource) {
      return;
    }
    ...
    try {
      ...
      const knowledgeBase = await updateKnowledgeBaseResource(editingKnowledgeBaseId, editKnowledgeBaseWorkspaceId, {
        name,
        slug,
      ...
      setManagementPanel(null);
      await refreshAdminDirectory();
      setStatusMessage(t("admin.status.knowledgeBaseUpdated", { name: knowledgeBase.name }));
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : t("admin.status.knowledgeBaseUpdateFailed"));
  ...
  }, [knowledgeBasePublicationStatusFilter, memberAccountFilter, memberRelationshipFilter, workspaceLifecycleFilter, auditEventFilter, selectedTenantId]);
  ...
  }, [errorMessage, isLoading, knowledgeBases.length, t, tenants.length, workspaces.length]);
  ...
  const filteredTenantGroups = useMemo(() => {
    ...
    return tenantWorkspaceGroups.filter((group) => group.tenant.id === selectedTenantId);
  ...
  const filteredWorkspaceIds = useMemo(
    () => new Set(filteredTenantGroups.flatMap((group) => group.workspaces.map((workspace) => workspace.id))),
    [filteredTenantGroups]
  );
  ...
  const filteredKnowledgeBases = useMemo(
    () => knowledgeBases.filter((knowledgeBase) => filteredWorkspaceIds.has(knowledgeBase.workspace_id)),
  ...
  const retrievalGovernanceSummary = useMemo(() => {
    const retrievalProfileById = new Map(retrievalProfiles.map((retrievalProfile) => [retrievalProfile.id, retrievalProfile] as const));
    const explicitAssignedKnowledgeBases = filteredKnowledgeBases.filter((knowledgeBase) => Boolean(knowledgeBase.retrieval_profile_id)).length;
    const defaultFallbackKnowledgeBases = filteredKnowledgeBases.filter((knowledgeBase) => !knowledgeBase.retrieval_profile_id).length;
    const knowledgeBasesUsingDisabledRetrievalProfile = filteredKnowledgeBases.filter((knowledgeBase) => {
      if (!knowledgeBase.retrieval_profile_id) {
        return false;
      }

      const retrievalProfile = retrievalProfileById.get(knowledgeBase.retrieval_profile_id);
      return Boolean(retrievalProfile && !retrievalProfile.is_enabled);
    }).length;
    const disabledBoundRetrievalProfiles = retrievalProfiles.filter(
      (retrievalProfile) => !retrievalProfile.is_enabled && retrievalProfile.bound_knowledge_base_count > 0
    ).length;

    return {
      totalProfiles: retrievalProfiles.length,
      enabledProfiles: retrievalProfiles.filter((retrievalProfile) => retrievalProfile.is_enabled).length,
  ...
  const retrievalProfileAssets = useMemo(() => {
    const fallbackKnowledgeBases = filteredKnowledgeBases.filter((knowledgeBase) => !knowledgeBase.retrieval_profile_id);

    return retrievalProfiles.map((retrievalProfile) => {
      const assignedKnowledgeBases = filteredKnowledgeBases.filter(
        (knowledgeBase) => knowledgeBase.retrieval_profile_id === retrievalProfile.id
      );
      const assignedWorkspaceCount = new Set(assignedKnowledgeBases.map((knowledgeBase) => knowledgeBase.workspace_id)).size;
      const publishedKnowledgeBaseCount = assignedKnowledgeBases.filter(
        (knowledgeBase) => knowledgeBase.publication_status === "published"
      ).length;
      const fallbackCoverageCount = defaultRetrievalProfile?.id === retrievalProfile.id ? fallbackKnowledgeBases.length : 0;
      const status: AdminWatchItem["status"] = !retrievalProfile.is_enabled && assignedKnowledgeBases.length > 0
        ? "attention"
      ...
      return {
        ...
        draftKnowledgeBaseCount: assignedKnowledgeBases.length - publishedKnowledgeBaseCount,
  ...
  const focusedDisabledModelEndpoint = useMemo(() => {
    const boundModelIds = new Set(
      scopedAgents
        .map((agent) => agent.model_endpoint_id)
        .filter((value): value is string => typeof value === "string" && value.length > 0)
    );
    return modelEndpoints.find((item) => !item.is_enabled && boundModelIds.has(item.id)) ?? null;
  }, [modelEndpoints, scopedAgents]);
  const focusedDisabledToolRegistration = useMemo(() => {
    const boundToolIds = new Set(scopedAgents.flatMap((agent) => agent.tool_registration_ids));
    return toolRegistrations.find((item) => !item.is_enabled && boundToolIds.has(item.id)) ?? null;
  }, [scopedAgents, toolRegistrations]);
  const focusedPendingMcpToolRegistration = useMemo(() => {
    const boundToolIds = new Set(scopedAgents.flatMap((agent) => agent.tool_registration_ids));
    return (
      toolRegistrations.find(
        (item) =>
          item.is_enabled &&
          item.transport_type === "mcp_reserved" &&
          boundToolIds.has(item.id) &&
          Boolean(item.connector_reference?.trim())
      ) ?? null
    );
  }, [scopedAgents, toolRegistrations]);
    ...
    return (
      toolRegistrations.find(
        (item) =>
          ...
          item.transport_type === "mcp_reserved" &&
          boundToolIds.has(item.id) &&
          !item.connector_reference?.trim()
      ) ?? null
    );
  }, [scopedAgents, toolRegistrations]);
  const focusedApprovalToolRegistration = useMemo(() => {
    const boundToolIds = new Set(scopedAgents.flatMap((agent) => agent.tool_registration_ids));
    return toolRegistrations.find((item) => item.requires_admin_approval && boundToolIds.has(item.id)) ?? null;
  }, [scopedAgents, toolRegistrations]);
  const focusedDisabledRetrievalProfile = useMemo(
    () => retrievalProfileAssets.find((asset) => asset.status === "attention")?.profile ?? null,
    [retrievalProfileAssets]
  );
  const focusedRuntimeSettingsHref = buildRuntimeGovernanceSettingsHref({
    tenantId: selectedTenantId === "all" ? null : selectedTenantId,
    disabledModelEndpointId: focusedDisabledModelEndpoint?.id ?? null,
    disabledToolRegistrationId: focusedDisabledToolRegistration?.id ?? null,
    pendingMcpToolRegistrationId: focusedPendingMcpToolRegistration?.id ?? null,
    pendingMcpConnectorReference: focusedPendingMcpToolRegistration?.connector_reference ?? null,
    reservedMcpToolRegistrationId: focusedReservedMcpToolRegistration?.id ?? null,
    reservedMcpConnectorReference: focusedReservedMcpToolRegistration?.connector_reference ?? null,
    disabledRetrievalProfileId: focusedDisabledRetrievalProfile?.id ?? null,
    approvalToolRegistrationId: focusedApprovalToolRegistration?.id ?? null
  });
  const focusedDisabledModelDefinitionsHref = focusedDisabledModelEndpoint
    ? buildAgentsHref({
        ...
        modelEndpointId: focusedDisabledModelEndpoint.id
      })
    : null;
  const focusedDisabledToolDefinitionsHref = focusedDisabledToolRegistration
    ? buildAgentsHref({
        ...
        readiness: "attention",
        issue: "tool_registration_disabled",
        toolRegistrationId: focusedDisabledToolRegistration.id
      })
    : null;
  const focusedApprovalToolDefinitionsHref = focusedApprovalToolRegistration
    ? buildAgentsHref({
        ...
        status: "active",
        issue: "tool_approval_required",
        toolRegistrationId: focusedApprovalToolRegistration.id
      })
    : null;
  const focusedDisabledRetrievalDefinitionsHref = focusedDisabledRetrievalProfile
    ? buildAgentsHref({
        tenantId: selectedTenantId === "all" ? null : selectedTenantId,
        status: "active",
        readiness: "attention",
  ...
  const scopedConversationMetrics = useMemo<ConversationMetrics>(() => {
    const scopedTenantIds = filteredTenantGroups.map((group) => group.tenant.id);
    ...
    const aggregatedMetrics = scopedTenantIds.reduce(
      (accumulator, tenantId) => {
        ...
        return {
          total_conversations: accumulator.total_conversations + metrics.total_conversations,
          active_conversations: accumulator.active_conversations + metrics.active_conversations,
    ...
    aggregatedMetrics.latest_activity_at =
      latestActivityCandidates.length > 0
        ? latestActivityCandidates.sort((left, right) => new Date(right).getTime() - new Date(left).getTime())[0]
        : null;
  ...
  const scopedAgentRunMetrics = useMemo<AgentRunMetricsResponse>(() => {
    ...
    const aggregatedMetrics = scopedTenantIds.reduce(
      (accumulator, tenantId) => {
        ...
        return {
          ...
          operations_runs: accumulator.operations_runs + metrics.operations_runs,
    ...
    aggregatedMetrics.latest_launched_at =
      latestLaunchCandidates.length > 0
        ? latestLaunchCandidates.sort((left, right) => new Date(right).getTime() - new Date(left).getTime())[0]
        : null;

    return aggregatedMetrics;
  }, [agentRunMetricsByTenantId, filteredTenantGroups]);

  const scopedAgentExecutionMetrics = useMemo<AgentExecutionMetricsResponse>(() => {
    const scopedTenantIds = filteredTenantGroups.map((group) => group.tenant.id);
    ...
    const aggregatedMetrics = scopedTenantIds.reduce(
      (accumulator, tenantId) => {
        ...
        return {
          total_executions: accumulator.total_executions + metrics.total_executions,
          queued_executions: accumulator.queued_executions + metrics.queued_executions,
          running_executions: accumulator.running_executions + metrics.running_executions,
          completed_executions: accumulator.completed_executions + metrics.completed_executions,
          failed_executions: accumulator.failed_executions + metrics.failed_executions,
    ...
    aggregatedMetrics.latest_execution_at =
      latestExecutionCandidates.length > 0
        ? latestExecutionCandidates.sort((left, right) => new Date(right).getTime() - new Date(left).getTime())[0]
        : null;
  ...
  const scopedDocumentMetrics = useMemo<DocumentMetrics>(() => {
    return filteredKnowledgeBases.reduce(
      (accumulator, knowledgeBase) => {
        ...
        return {
          total_documents: accumulator.total_documents + metrics.total_documents,
          completed_documents: accumulator.completed_documents + metrics.completed_documents,
          active_documents: accumulator.active_documents + metrics.active_documents,
          failed_documents: accumulator.failed_documents + metrics.failed_documents
        };
      },
  ...
  const recentScopedAgentExecutions = useMemo(
    () =>
      filteredTenantGroups
        .flatMap((group) => recentAgentExecutionsByTenantId[group.tenant.id] ?? [])
  ...
  const tenantAgentRuntimeActivity = useMemo(
    () =>
      filteredTenantGroups
        .map((group) => {
          const metrics = agentRunMetricsByTenantId[group.tenant.id] ?? EMPTY_AGENT_RUN_METRICS;
        ...
        .sort((left, right) => {
          ...
          const rightLatest = right.metrics.latest_launched_at ? new Date(right.metrics.latest_launched_at).getTime() : 0;
          const leftLatest = left.metrics.latest_launched_at ? new Date(left.metrics.latest_launched_at).getTime() : 0;
          return rightLatest - leftLatest;
        }),
    [agentRunMetricsByTenantId, filteredTenantGroups, recentAgentRunsByTenantId]
  );
  ...
  const tenantAgentExecutionActivity = useMemo(
    () =>
      filteredTenantGroups
        .map((group) => {
          const metrics = agentExecutionMetricsByTenantId[group.tenant.id] ?? EMPTY_AGENT_EXECUTION_METRICS;
        ...
        .sort((left, right) => {
          if (right.metrics.total_executions !== left.metrics.total_executions) {
            return right.metrics.total_executions - left.metrics.total_executions;
          }

          const rightLatest = right.metrics.latest_execution_at ? new Date(right.metrics.latest_execution_at).getTime() : 0;
          const leftLatest = left.metrics.latest_execution_at ? new Date(left.metrics.latest_execution_at).getTime() : 0;
          return rightLatest - leftLatest;
        }),
    [agentExecutionMetricsByTenantId, filteredTenantGroups]
  );
  ...
  const tenantWorkflowActivity = useMemo(
    () =>
      filteredTenantGroups
        .map((group) => {
          const metrics = workflowMetricsByTenantId[group.tenant.id] ?? EMPTY_WORKFLOW_METRICS;
          const primaryWorkspace = group.workspaces[0] ?? null;
          const primaryKnowledgeBase =
            primaryWorkspace
              ? knowledgeBases.find((knowledgeBase) => knowledgeBase.workspace_id === primaryWorkspace.id) ?? null
              : null;
          ...
          return {
            ...
            workspaceWorkflowHref: buildAdminWorkspaceHref("overview", {
              ...
              workflowStatus: metrics.failed_runs > 0 ? "failed" : null
            })
          };
        })
        .sort((left, right) => {
          if (right.metrics.failed_runs !== left.metrics.failed_runs) {
            return right.metrics.failed_runs - left.metrics.failed_runs;
          }
      ...
      filteredTenantGroups
        .map((group) => {
          const groupKnowledgeBases = knowledgeBases.filter((knowledgeBase) =>
            group.workspaces.some((workspace) => workspace.id === knowledgeBase.workspace_id)
          );
          const metrics = groupKnowledgeBases.reduce(
            (accumulator, knowledgeBase) => {
              const nextMetrics = documentMetricsByKnowledgeBaseId[knowledgeBase.id];
              if (!nextMetrics) {
                return accumulator;
              }

              return {
                total_documents: accumulator.total_documents + nextMetrics.total_documents,
                completed_documents: accumulator.completed_documents + nextMetrics.completed_documents,
                active_documents: accumulator.active_documents + nextMetrics.active_documents,
                failed_documents: accumulator.failed_documents + nextMetrics.failed_documents
              };
            },
            { ...EMPTY_DOCUMENT_METRICS }
          );
        ...
        .sort((left, right) => {
          if (right.metrics.failed_documents !== left.metrics.failed_documents) {
            return right.metrics.failed_documents - left.metrics.failed_documents;
          }
          if (right.metrics.active_documents !== left.metrics.active_documents) {
            return right.metrics.active_documents - left.metrics.active_documents;
          }
  ...
  const tenantChatActivity = useMemo(
    () =>
      filteredTenantGroups
        .map((group) => {
          const metrics = conversationMetricsByTenantId[group.tenant.id] ?? EMPTY_CONVERSATION_METRICS;
          const primaryWorkspace = group.workspaces[0] ?? null;
          const primaryKnowledgeBase =
            primaryWorkspace
              ? knowledgeBases.find((knowledgeBase) => knowledgeBase.workspace_id === primaryWorkspace.id) ?? null
        ...
        .sort((left, right) => {
          ...
          const rightLatest = right.metrics.latest_activity_at ? new Date(right.metrics.latest_activity_at).getTime() : 0;
          const leftLatest = left.metrics.latest_activity_at ? new Date(left.metrics.latest_activity_at).getTime() : 0;
          return rightLatest - leftLatest;
        }),
    [conversationMetricsByTenantId, filteredTenantGroups, knowledgeBases]
  );

  const workflowSignals = useMemo(() => {
    const tenantsWithFailures = tenantWorkflowActivity.filter((item) => item.metrics.failed_runs > 0).length;
    const tenantsWithQueuePressure = tenantWorkflowActivity.filter((item) => item.queuePressure > 0).length;
    const tenantsWithRetries = tenantWorkflowActivity.filter((item) => item.metrics.retry_runs > 0).length;
  ...
  const documentSignals = useMemo(() => {
    const tenantsWithFailedDocuments = tenantDocumentActivity.filter((item) => item.metrics.failed_documents > 0).length;
    const tenantsWithActiveIntake = tenantDocumentActivity.filter((item) => item.metrics.active_documents > 0).length;
  ...
  const chatSignals = useMemo(() => {
    ...
    const staleTenantCount = tenantChatActivity.filter((item) => {
      ...
      return now - new Date(item.metrics.latest_activity_at).getTime() > 1000 * 60 * 60 * 24 * 7;
    }).length;
    const activeTenant = tenantChatActivity[0] ?? null;
    const idleConversationTenantCount = tenantChatActivity.filter(
      (item) => item.metrics.total_conversations > 0 && item.metrics.active_conversations === 0
    ).length;
  ...
  const searchedWorkspaces = useMemo(() => {
    ...
    return filteredWorkspaces.filter((workspace) => {
      const tenant = tenants.find((tenantItem) => tenantItem.id === workspace.tenant_id);
  ...
  const searchedKnowledgeBases = useMemo(() => {
    const candidateKnowledgeBases = filteredKnowledgeBases.filter((knowledgeBase) => {
      if (!searchedWorkspaceIds.has(knowledgeBase.workspace_id)) {
        return false;
      }

      if (retrievalProfileFilter !== "all") {
        if (retrievalProfileFilter === DEFAULT_RETRIEVAL_PROFILE_FILTER_VALUE) {
          if (knowledgeBase.retrieval_profile_id) {
            return false;
          }
        } else if (retrievalProfileFilter === DISABLED_RETRIEVAL_PROFILE_FILTER_VALUE) {
          if (!knowledgeBase.retrieval_profile_id) {
            return false;
          }

          const assignedRetrievalProfile = retrievalProfiles.find(
            (retrievalProfile) => retrievalProfile.id === knowledgeBase.retrieval_profile_id
          ) ?? null;
          if (!assignedRetrievalProfile || assignedRetrievalProfile.is_enabled) {
            return false;
          }
        } else {
          const effectiveRetrievalProfileId = knowledgeBase.retrieval_profile_id ?? defaultRetrievalProfile?.id ?? null;
    ...
    return candidateKnowledgeBases.filter((knowledgeBase) => {
      const workspace = workspaces.find((workspaceItem) => workspaceItem.id === knowledgeBase.workspace_id);
      const tenant = tenants.find((tenantItem) => tenantItem.id === knowledgeBase.tenant_id);
      const assignedRetrievalProfile = knowledgeBase.retrieval_profile_id
        ? retrievalProfiles.find((retrievalProfile) => retrievalProfile.id === knowledgeBase.retrieval_profile_id) ?? null
        : null;
      const effectiveRetrievalProfile = assignedRetrievalProfile ?? defaultRetrievalProfile;
      const searchableText = [
        knowledgeBase.name,
        knowledgeBase.slug,
  ...
  }, [defaultRetrievalProfile, filteredKnowledgeBases, normalizedSearchQuery, retrievalProfileFilter, retrievalProfiles, searchedWorkspaceIds, tenants, workspaces]);

  const filteredAgents = useMemo(
    () => agents.filter((agent) => selectedTenantId === "all" || agent.tenant_id === selectedTenantId),
  ...
  const searchedAgents = useMemo(() => {
    ...
    return filteredAgents.filter((agent) => {
      const tenant = tenants.find((tenantItem) => tenantItem.id === agent.tenant_id);
      const searchableText = [
        tenant?.name,
        tenant?.slug,
        agent.name,
  ...
  const filteredUsers = useMemo(
    () =>
      users.filter(
        (user) =>
          selectedTenantId === "all" || user.memberships.some((membership) => membership.tenant_id === selectedTenantId)
      ),
  ...
  const searchedUsers = useMemo(() => {
    ...
    return filteredUsers.filter((user) => {
      const searchableText = [
        ...
        user.email,
        user.role,
        ...user.memberships.flatMap((membership) => [membership.tenant_name, membership.tenant_slug, membership.membership_status])
      ]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
  ...
  const adminWatchItems = useMemo<AdminWatchItem[]>(() => {
    const archivedWorkspaceCount = filteredWorkspaces.filter((workspace) => workspace.is_archived).length;
    const firstArchivedWorkspace = filteredWorkspaces.find((workspace) => workspace.is_archived) ?? null;
    ...
    const draftAgentCount = filteredAgents.filter((agent) => agent.status === "draft").length;
    const activeAgentsWithoutScopeCount = governancePosture.activeAgentsWithoutScope;
    const invitedMembershipCount = filteredUsers.reduce(
      (count, user) => count + user.memberships.filter((membership) => membership.membership_status === "invited").length,
      0
    );
          ...
          membership:
            user.memberships.find(
              (membership) =>
                ...
                (selectedTenantId === "all" || membership.tenant_id === selectedTenantId)
            ) ?? null
        }))
    ...
    const fallbackPrimaryKnowledgeBase =
      filteredKnowledgeBases.find((knowledgeBase) => knowledgeBase.workspace_id === fallbackPrimaryWorkspace?.id) ??
    ...
    return [
        ...
        detail:
          scopedWorkflowMetrics.failed_runs > 0
            ? t("admin.watchlist.workflowRecoveryPressureDetail", {
                count: String(scopedWorkflowMetrics.failed_runs),
                tenantCount: String(workflowSignals.tenantsWithFailures)
              })
            : t("admin.watchlist.workflowRecoveryHealthy"),
        status: scopedWorkflowMetrics.failed_runs > 0 ? "attention" : workflowSignals.tenantsWithQueuePressure > 0 ? "review" : "healthy",
      ...
      },
      {
        ...
        actionHref:
          ...
          buildAdminWorkspaceHref("overview", {
            ...
            documentStatus: scopedDocumentMetrics.failed_documents > 0 ? "failed" : null
          })
      },
      {
        ...
        detail:
          invitedMembershipCount > 0
            ? t("admin.watchlist.memberActivationQueueDetail", { count: String(invitedMembershipCount) })
        ...
        actionHref:
          firstInvitedMemberRecord?.membership
      ...
      },
      {
        ...
        detail:
          archivedWorkspaceCount > 0
            ? t("admin.watchlist.workspaceLifecycleReviewDetail", { count: String(archivedWorkspaceCount) })
      ...
      },
      {
        ...
        detail:
          draftKnowledgeBaseCount > 0
            ? t("admin.watchlist.knowledgePublicationDetail", { count: String(draftKnowledgeBaseCount) })
      ...
      },
      {
        ...
        detail:
          publishedKnowledgeBaseCount > 0
            ? t("admin.watchlist.publishedRetrievalDetail", { count: String(publishedKnowledgeBaseCount) })
      ...
      },
      {
        ...
        detail:
          retrievalGovernanceSummary.knowledgeBasesUsingDisabledRetrievalProfile > 0
            ? t("admin.watchlist.retrievalGovernanceDisabledDetail", {
                count: String(retrievalGovernanceSummary.knowledgeBasesUsingDisabledRetrievalProfile)
              })
            : retrievalGovernanceSummary.defaultFallbackKnowledgeBases > 0
              ? t("admin.watchlist.retrievalGovernanceDefaultFallbackDetail", {
                  count: String(retrievalGovernanceSummary.defaultFallbackKnowledgeBases)
                })
          ...
          governancePosture.agentsUsingDisabledRetrievalProfile > 0
            ...
            : governancePosture.agentsMissingRetrievalProfile > 0
              ? t("admin.watchlist.runtimeGovernanceMissingRetrievalDetail", {
                  count: String(governancePosture.agentsMissingRetrievalProfile)
                })
      ...
      },
      {
        ...
        detail:
          activeAgentsWithoutScopeCount > 0
            ? t("admin.watchlist.agentGovernanceDetail", { count: String(activeAgentsWithoutScopeCount) })
            : draftAgentCount > 0
              ? t("admin.watchlist.agentDraftDetail", { count: String(draftAgentCount) })
              : t("admin.watchlist.agentGovernanceHealthy"),
        status: activeAgentsWithoutScopeCount > 0 ? "attention" : draftAgentCount > 0 ? "review" : "healthy",
        actionLabel: t("admin.watchlist.reviewAgents"),
        actionHref: buildAgentsHref({
          tenantId: selectedTenantId,
          status: activeAgentsWithoutScopeCount > 0 ? "active" : draftAgentCount > 0 ? "draft" : "all"
        })
      }
    ];
  }, [
    ...
    scopedWorkflowMetrics.failed_runs,
    t,
  ...
  const metrics = useMemo(
    () => [
      ...
      },
      {
        ...
        value: filteredWorkspaces.filter((workspace) => !workspace.is_archived).length,
      ...
      },
      {
        ...
        value: filteredKnowledgeBases.filter((knowledgeBase) => knowledgeBase.publication_status === "published").length,
      ...
      },
      {
        ...
        value: filteredAgents.filter((agent) => agent.status === "active").length,
      ...
      },
      {
        ...
        value: adminWatchItems.filter((item) => item.status !== "healthy").length,
        hint: t("admin.metrics.pendingControlsHint"),
        href: buildAdminHref({
          section: "security",
          tenantId: selectedTenantId,
          workspaceLifecycleFilter,
          knowledgeBasePublicationStatusFilter: knowledgeBasePublicationStatusFilter === "all" ? "draft" : knowledgeBasePublicationStatusFilter
        })
      }
    ],
    [
  ...
  const accessSummaryItems = useMemo(
    () => {
      const invitedMembershipCount = searchedUsers.reduce(
        (count, user) => count + user.memberships.filter((membership) => membership.membership_status === "invited").length,
        0
      );
      const suspendedMembershipCount = searchedUsers.reduce(
        (count, user) => count + user.memberships.filter((membership) => membership.membership_status === "suspended").length,
        0
      );
    ...
    [auditEvents.length, searchedUsers, t]
  );
  ...
  const invitedActivationQueue = useMemo(() => {
    return searchedUsers
        ...
        user.memberships
          ...
          .map((membership) => {
            const invitationCredential =
              revealedInvitationByMembershipId[membership.id] ??
              null;
            const invitationExpiresAt =
              invitationCredential?.invitation_expires_at ?? membership.invitation_expires_at ?? null;
            ...
            return {
              ...
              invitationIssuedAt: invitationCredential?.invited_at ?? membership.invited_at ?? null,
              invitationExpiresAt
            };
          })
      )
      .sort((left, right) => {
        ...
        const leftExpiresAt = left.invitationExpiresAt ? new Date(left.invitationExpiresAt).getTime() : Number.MAX_SAFE_INTEGER;
        const rightExpiresAt = right.invitationExpiresAt ? new Date(right.invitationExpiresAt).getTime() : Number.MAX_SAFE_INTEGER;
        if (leftExpiresAt !== rightExpiresAt) {
          return leftExpiresAt - rightExpiresAt;
        }

        const leftInvitedAt = left.invitationIssuedAt ? new Date(left.invitationIssuedAt).getTime() : 0;
        const rightInvitedAt = right.invitationIssuedAt ? new Date(right.invitationIssuedAt).getTime() : 0;
  ...
  const securitySummaryItems = useMemo(() => {
    const now = Date.now();
    const dormantThresholdMs = 1000 * 60 * 60 * 24 * 30;
    const expiringSoonThresholdMs = 1000 * 60 * 60 * 24 * 3;

    const activeAccountCount = searchedUsers.filter((user) => user.is_active).length;
    const dormantAccountCount = searchedUsers.filter((user) => {
      if (!user.is_active) {
        return false;
      }
      ...
      return now - new Date(user.last_signed_in_at).getTime() > dormantThresholdMs;
    }).length;

    const expiringInvitationCount = searchedUsers.reduce((count, user) => {
      return (
        count +
        user.memberships.filter((membership) => {
          ...
          return expiresAt >= now && expiresAt - now <= expiringSoonThresholdMs;
        }).length
      );
    }, 0);

    const expiredInvitationCount = searchedUsers.reduce((count, user) => {
      return (
        count +
        user.memberships.filter((membership) => {
          if (membership.membership_status !== "invited" || !membership.invitation_expires_at) {
          ...
          return new Date(membership.invitation_expires_at).getTime() < now;
        }).length
      );
    }, 0);

    const sensitiveEventCount = auditEvents.filter((event) =>
      ["invitation_revoked", "membership_suspended", "membership_deleted"].includes(event.event_type)
    ).length;
    ...
    return [
      ...
      },
      {
        ...
        value: dormantAccountCount,
        hint: t("admin.securitySummary.dormantAccountsHint")
      },
      {
        label: t("admin.securitySummary.expiringInvitations"),
        value: expiringInvitationCount + expiredInvitationCount,
        hint: t("admin.securitySummary.expiringInvitationsHint")
      },
      {
        label: t("admin.securitySummary.sensitiveEvents"),
        value: sensitiveEventCount,
        hint: t("admin.securitySummary.sensitiveEventsHint")
      }
    ];
  }, [auditEvents, searchedUsers, t]);

  const securityWatchItems = useMemo<AdminWatchItem[]>(() => {
    const now = Date.now();
    const expiringSoonThresholdMs = 1000 * 60 * 60 * 24 * 3;
    const firstExpiredInvitationRecord =
      filteredUsers
        .map((user) => ({
          user,
          membership:
        ...
        .find((item) => item.membership !== null) ?? null;
    const firstExpiringInvitationRecord =
      filteredUsers
        .map((user) => ({
          user,
          membership:
    ...
    const firstDormantUser =
      filteredUsers.find((user) => {
        ...
        return now - new Date(user.last_signed_in_at).getTime() > 1000 * 60 * 60 * 24 * 30;
      }) ?? null;
    const firstSuspendedMembershipRecord =
      filteredUsers
        .map((user) => ({
          user,
          membership:
            user.memberships.find((membership) => membership.membership_status === "suspended") ?? null
        }))
        .find((item) => item.membership !== null) ?? null;

    const expiredInvitationCount = filteredUsers.reduce((count, user) => {
      return (
        count +
        user.memberships.filter((membership) => {
          if (membership.membership_status !== "invited" || !membership.invitation_expires_at) {
    ...
    const expiringInvitationCount = filteredUsers.reduce((count, user) => {
      return (
        count +
        user.memberships.filter((membership) => {
          if (membership.membership_status !== "invited" || !membership.invitation_expires_at) {
    ...
    const dormantAccountCount = filteredUsers.filter((user) => {
      if (!user.is_active) {
        return false;
      }
    ...
    const suspendedMembershipCount = filteredUsers.reduce(
      (count, user) => count + user.memberships.filter((membership) => membership.membership_status === "suspended").length,
      0
    );
    ...
    return [
      {
        ...
        detail:
          expiredInvitationCount > 0
            ? t("admin.securityWatch.expiredInvitationsDetail", { count: String(expiredInvitationCount) })
            : t("admin.securityWatch.expiredInvitationsHealthy"),
        status: expiredInvitationCount > 0 ? "attention" : "healthy",
        actionLabel: t("admin.securityWatch.reviewInvitations"),
        actionHref:
          firstExpiredInvitationRecord?.membership
            ? buildMemberGovernanceHref(firstExpiredInvitationRecord.user, {
                tenantId: firstExpiredInvitationRecord.membership.tenant_id,
                memberRelationshipFilter: "invited"
              })
            : buildAdminHref({
                tenantId: selectedTenantId,
      ...
      },
      {
        ...
        detail:
          expiringInvitationCount > 0
            ? t("admin.securityWatch.expiringInvitationsDetail", { count: String(expiringInvitationCount) })
            : t("admin.securityWatch.expiringInvitationsHealthy"),
        status: expiringInvitationCount > 0 ? "review" : "healthy",
        actionLabel: t("admin.securityWatch.reviewInvitations"),
        actionHref:
          firstExpiringInvitationRecord?.membership
            ? buildMemberGovernanceHref(firstExpiringInvitationRecord.user, {
                tenantId: firstExpiringInvitationRecord.membership.tenant_id,
                memberRelationshipFilter: "invited"
              })
            : buildAdminHref({
                tenantId: selectedTenantId,
      ...
      },
      {
        ...
        detail:
          dormantAccountCount > 0
            ? t("admin.securityWatch.dormantAccountsDetail", { count: String(dormantAccountCount) })
            : t("admin.securityWatch.dormantAccountsHealthy"),
        status: dormantAccountCount > 0 ? "review" : "healthy",
        actionLabel: t("admin.securityWatch.reviewAccounts"),
        actionHref:
          firstDormantUser
            ? buildMemberGovernanceHref(firstDormantUser, {
                tenantId: selectedTenantId,
              })
            : buildAdminHref({
                tenantId: selectedTenantId,
      ...
      },
      {
        ...
        detail:
          suspendedMembershipCount > 0
            ? t("admin.securityWatch.suspendedMembershipsDetail", { count: String(suspendedMembershipCount) })
            : t("admin.securityWatch.suspendedMembershipsHealthy"),
        status: suspendedMembershipCount > 0 ? "review" : "healthy",
        actionLabel: t("admin.securityWatch.reviewSuspended"),
        actionHref:
          firstSuspendedMembershipRecord?.membership
            ? buildMemberGovernanceHref(firstSuspendedMembershipRecord.user, {
                tenantId: firstSuspendedMembershipRecord.membership.tenant_id,
  ...
  }, [filteredUsers, selectedTenantId, t]);
  ...
  const currentActorSecurityPosture = useMemo(() => {
    ...
    const currentActor = users.find((user) => user.id === session.userId) ?? null;
    if (!currentActor) {
      return null;
    }

    const now = Date.now();
    const activeMembershipCount = currentActor.memberships.filter(
      (membership) => membership.membership_status === "active"
    ).length;
    const invitedMembershipCount = currentActor.memberships.filter(
      (membership) => membership.membership_status === "invited"
    ).length;
    const suspendedMembershipCount = currentActor.memberships.filter(
      (membership) => membership.membership_status === "suspended"
    ).length;
    const expiredInvitationCount = currentActor.memberships.filter(
      (membership) =>
        membership.membership_status === "invited" &&
        membership.invitation_expires_at &&
        new Date(membership.invitation_expires_at).getTime() < now
    ).length;
    const expiringInvitationCount = currentActor.memberships.filter((membership) => {
      if (membership.membership_status !== "invited" || !membership.invitation_expires_at) {
        return false;
      }
    ...
    () => [
      ...
      },
      {
        ...
        description: t("admin.overviewLanes.access.description", {
          memberCount: String(searchedUsers.length)
        }),
        value: t("admin.overviewLanes.access.value", {
          invitedCount: String(
            searchedUsers.reduce(
              (count, user) =>
                count + user.memberships.filter((membership) => membership.membership_status === "invited").length,
              0
            )
          )
        }),
      ...
      },
      {
        ...
        description: t("admin.overviewLanes.security.description", {
          eventCount: String(auditEvents.length)
        }),
        value: t("admin.overviewLanes.security.value", {
          count: String(securityWatchItems.filter((item) => item.status !== "healthy").length)
        }),
        href: buildAdminHref({
          tenantId: selectedTenantId,
          section: "security",
  ...
  function buildWorkspaceContextHref(workspace: Workspace, view: "chat" | "documents" | "workflows") {
    const firstKnowledgeBase =
      knowledgeBases.find((knowledgeBase) => knowledgeBase.workspace_id === workspace.id) ?? null;
  ...
  function buildKnowledgeBaseContextHref(knowledgeBase: KnowledgeBase, view: "documents" | "workflows") {
    return buildAdminWorkspaceHref(adminSection, {
      view,
  ...
  const scopedPrimaryKnowledgeBase =
    scopedPrimaryWorkspace
      ? knowledgeBases.find((knowledgeBase) => knowledgeBase.workspace_id === scopedPrimaryWorkspace.id) ?? null
      : null;
  const chatScopeHref = buildAdminWorkspaceHref(adminSection, {
    view: "chat",
    tenantId: selectedTenantId === "all" ? scopedPrimaryWorkspace?.tenant_id ?? null : selectedTenantId,
    workspaceId: scopedPrimaryWorkspace?.id ?? null,
    knowledgeBaseId: scopedPrimaryKnowledgeBase?.id ?? null
  });
  const staleChatScopeHref = chatSignals.activeTenant?.openHref ?? chatScopeHref;
  const idleConversationScopeHref =
    tenantChatActivity.find((item) => item.metrics.total_conversations > 0 && item.metrics.active_conversations === 0)?.openHref ??
    chatScopeHref;
  ...
  const runtimeRoutes = useMemo<AdminRuntimeRoute[]>(() => {
    return searchedAgents
      ...
      .map((agent) => {
        ...
        const workspaceId = resolvedScope.workspaceId || scopedPrimaryWorkspace?.id || null;
        const knowledgeBaseId = resolvedScope.knowledgeBaseId || scopedPrimaryKnowledgeBase?.id || null;
        ...
        const resolvedKnowledgeBase = knowledgeBases.find((knowledgeBase) => knowledgeBase.id === knowledgeBaseId) ?? null;
        const assignedRetrievalProfileId = resolvedKnowledgeBase?.retrieval_profile_id?.trim() ?? "";
        const selectedRetrievalProfile = assignedRetrievalProfileId
          ? retrievalProfiles.find((retrievalProfile) => retrievalProfile.id === assignedRetrievalProfileId) ?? null
          : null;
          ...
          agent.mode === "workflow_recovery" || !scopeResolved
            ...
            : !resolvedRetrievalProfile
              ...
              : !resolvedRetrievalProfile.is_enabled
                ? ("disabled" as const)
                : null;
        const launchReady = agent.mode === "workflow_recovery" || (scopeResolved && retrievalIssue === null);
        ...
        const governanceHref =
          resolvedKnowledgeBase && retrievalIssue
            ? buildKnowledgeBaseGovernanceHref(resolvedKnowledgeBase, {
                retrievalProfileFilter:
                  retrievalIssue === "disabled" ? DISABLED_RETRIEVAL_PROFILE_FILTER_VALUE : null
              })
            : buildAdminHref({
                tenantId: agent.tenant_id,
                section: "access"
        ...
        const recommendedRunRecord: AgentRunRecordInput | null = launchReady
          ? {
              ...
              target_surface:
                agent.mode === "workflow_recovery" ? "operations" : agent.mode === "document_intake" ? "documents" : "chat",
              handoff_intent: agent.mode === "workflow_recovery" ? "workflow_recovery" : "agent_brief",
  ...
  const focusedRuntimeRoute = useMemo(
    () => runtimeRoutes.find((item) => item.launchReady) ?? runtimeRoutes[0] ?? null,
    [runtimeRoutes]
  );
  ...
  const runtimeTaskPacket = useMemo(() => {
    if (!focusedRuntimeRoute) {
      return {
        ...
        primaryActionHref: buildAgentsHref({ tenantId: selectedTenantId === "all" ? null : selectedTenantId, status: "active" }),
    ...
    return {
      detail: focusedRuntimeRoute.launchReady
        ...
        : focusedRuntimeRoute.retrievalIssue === "disabled"
          ? t("admin.runtimeTaskPacket.retrievalDisabledDetail", {
              profile: focusedRuntimeRoute.retrievalProfile?.name ?? t("agents.dependencies.noRetrievalProfile")
            })
      ...
      prompt: focusedRuntimeRoute.prompt.trim().length ? focusedRuntimeRoute.prompt : t("admin.runtimeTaskPacket.noPrompt"),
      secondaryActions: [
        {
        ...
        },
        {
          ...
          runRecord: {
            ...
            knowledge_base_id: focusedRuntimeRoute.resolvedKnowledgeBase?.id ?? null,
            target_surface: "admin",
            handoff_intent: "agent_brief",
            trigger_source: "admin",
            launch_prompt: focusedRuntimeRoute.prompt.trim().length ? focusedRuntimeRoute.prompt : null
      ...
      statusTone: focusedRuntimeRoute.launchReady ? ("healthy" as const) : ("review" as const),
      summaryItems: [
        {
          label: t("admin.runtimeTaskPacket.fields.mode"),
        ...
        {
          label: t("admin.runtimeTaskPacket.fields.workspace"),
          value: focusedRuntimeRoute.resolvedWorkspace?.name ?? t("admin.runtimeTaskPacket.pending")
        },
        {
          label: t("admin.runtimeTaskPacket.fields.knowledgeBase"),
          value: focusedRuntimeRoute.resolvedKnowledgeBase?.name ?? t("admin.runtimeTaskPacket.pending")
        }
      ],
  ...
  const adminExecutionPackets = useMemo<AdminExecutionPacket[]>(() => {
    const activeAgentCount = filteredAgents.filter((agent) => agent.status === "active").length;
    const scopedRuntimeRoutes = runtimeRoutes.filter((item) => item.launchReady);
    const recoveryRuntimeRoutes = runtimeRoutes.filter(
      (item) => item.agent.mode === "workflow_recovery"
    );
    const scopedRecoveryRuntimeRoutes = recoveryRuntimeRoutes.filter((item) => item.launchReady);
    const draftKnowledgeBaseCount = filteredKnowledgeBases.filter(
      (knowledgeBase) => knowledgeBase.publication_status !== "published"
    ).length;
    const invitedMembershipCount = filteredUsers.reduce(
    ...
    const scopedTenantId = selectedTenantId === "all" ? scopedPrimaryWorkspace?.tenant_id ?? null : selectedTenantId;
    ...
    return [
      {
        ...
        status: scopedPrimaryWorkspace && scopedPrimaryKnowledgeBase ? "review" : "attention",
      ...
      },
      {
        ...
        detail:
          draftKnowledgeBaseCount > 0
            ? t("admin.executionPackets.publication.draftDetail", { count: String(draftKnowledgeBaseCount) })
        ...
        primaryActionLabel: t("admin.executionPackets.publication.primaryAction"),
      ...
      },
      {
        ...
        detail:
          scopedRuntimeRoutes.length > 0
            ...
            : activeAgentCount > 0
              ? t("admin.executionPackets.runtime.pendingDetail", { total: String(activeAgentCount) })
        ...
        primaryActionRunRecord: scopedRuntimeRoutes[0]?.recommendedRunRecord ?? null,
        secondaryActions: [
          {
          ...
          },
          {
            ...
            runRecord:
              scopedRuntimeRoutes[0]
                ? ({
                    tenant_id: scopedRuntimeRoutes[0].agent.tenant_id,
                    agent_definition_id: scopedRuntimeRoutes[0].agent.id,
                    workspace_id: scopedRuntimeRoutes[0].resolvedWorkspace?.id ?? null,
                    knowledge_base_id: scopedRuntimeRoutes[0].resolvedKnowledgeBase?.id ?? null,
                    target_surface: "operations",
                    handoff_intent: "agent_brief",
                    trigger_source: "admin",
                    launch_prompt: scopedRuntimeRoutes[0].prompt.trim().length ? scopedRuntimeRoutes[0].prompt : null
                  } as AgentRunRecordInput)
                : null
          }
        ]
      },
      {
        ...
        primaryActionLabel: t("admin.executionPackets.recoveryRuntime.primaryAction"),
        primaryActionHref:
        ...
        primaryActionRunRecord: scopedRecoveryRuntimeRoutes[0]?.recommendedRunRecord ?? null,
        secondaryActions: [
          {
            label: t("admin.executionPackets.recoveryRuntime.secondaryDefinitions"),
            href: buildAgentsHref({
              tenantId: selectedTenantId,
              status: "active",
              mode: "workflow_recovery"
          ...
          {
            label: t("admin.executionPackets.recoveryRuntime.secondaryOperations"),
            href: buildOperationsHref({
              tenantId: scopedTenantId,
              lane: "failed",
              status: "failed"
            ...
            runRecord:
              scopedRecoveryRuntimeRoutes[0]
                ? ({
                    tenant_id: scopedRecoveryRuntimeRoutes[0].agent.tenant_id,
                    agent_definition_id: scopedRecoveryRuntimeRoutes[0].agent.id,
                    workspace_id: scopedRecoveryRuntimeRoutes[0].resolvedWorkspace?.id ?? null,
                    knowledge_base_id: scopedRecoveryRuntimeRoutes[0].resolvedKnowledgeBase?.id ?? null,
                    target_surface: "operations",
                    handoff_intent: "workflow_recovery",
                    trigger_source: "admin",
                    launch_prompt:
                      scopedRecoveryRuntimeRoutes[0].prompt.trim().length ? scopedRecoveryRuntimeRoutes[0].prompt : null
                  } as AgentRunRecordInput)
                : null
          }
        ]
      },
      {
        ...
        detail:
          invitedMembershipCount > 0
            ? t("admin.executionPackets.access.pendingDetail", { count: String(invitedMembershipCount) })
        ...
        primaryActionHref:
          firstInvitedMembershipRecord?.membership
    ...
    <ConsoleShell activeHref="/admin">
      ...
      <div className="mx-auto flex w-full max-w-[1680px] flex-col gap-6">
        <ConsolePageHeader
          actions={
            <>
              ...
              <Select disabled={isLoading} onValueChange={setWorkspaceLifecycleFilter} value={workspaceLifecycleFilter}>
                <SelectTrigger className="min-w-[220px] rounded-xl border-slate-200 bg-white">
                  <SelectValue placeholder={t("admin.filters.workspaceLifecycle")} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">{t("admin.filters.allWorkspaces")}</SelectItem>
                  <SelectItem value="active">{t("admin.filters.activeOnly")}</SelectItem>
                  <SelectItem value="archived">{t("admin.filters.archivedOnly")}</SelectItem>
                </SelectContent>
              </Select>

              <Select disabled={isLoading} onValueChange={setKnowledgeBasePublicationStatusFilter} value={knowledgeBasePublicationStatusFilter}>
                <SelectTrigger className="min-w-[220px] rounded-xl border-slate-200 bg-white">
                  <SelectValue placeholder={t("admin.filters.knowledgePublication")} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">{t("admin.filters.allKnowledgeBases")}</SelectItem>
                  <SelectItem value="published">{t("admin.filters.publishedOnly")}</SelectItem>
                  <SelectItem value="draft">{t("admin.filters.draftOnly")}</SelectItem>
                </SelectContent>
              </Select>

              <Select disabled={isLoading || retrievalProfiles.length === 0} onValueChange={setRetrievalProfileFilter} value={retrievalProfileFilter}>
                <SelectTrigger className="min-w-[220px] rounded-xl border-slate-200 bg-white">
                  <SelectValue placeholder={t("admin.filters.retrievalProfile")} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">{t("admin.filters.allRetrievalProfiles")}</SelectItem>
                  <SelectItem value={DEFAULT_RETRIEVAL_PROFILE_FILTER_VALUE}>{t("admin.filters.defaultFallbackRetrievalProfile")}</SelectItem>
                  <SelectItem value={DISABLED_RETRIEVAL_PROFILE_FILTER_VALUE}>{t("admin.filters.disabledAssignedRetrievalProfile")}</SelectItem>
              ...
              <Button
                ...
                disabled={isLoading}
                onClick={() => void refreshAdminDirectory()}
                type="button"
                variant="outline"
              >
                <RefreshCw className={cn("h-4 w-4", isLoading && "animate-spin")} />
                {isLoading ? t("admin.filters.refreshingDirectory") : t("admin.filters.refreshDirectory")}
              </Button>
            </>
          }
        ...
        <ConsoleSurface className="p-3">
          <ConsoleSurfaceHeader
            description={t("admin.sections.description")}
        ...
        <ConsoleSurface className="p-3">
          <div className="grid gap-3 xl:grid-cols-5">
            {metrics.map((metric) => (
              <Card className="rounded-[20px] border-slate-100 shadow-none" key={metric.label}>
                <CardContent className="p-6">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{metric.label}</div>
                  <div className="mt-4 text-[34px] font-semibold tracking-tight text-slate-950">{metric.value}</div>
                  <div className="mt-3 text-sm leading-6 text-slate-500">{metric.hint}</div>
                  <div className="mt-4">
                    <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                      <Link href={metric.href}>{t("admin.metrics.openScope")}</Link>
                    </Button>
                  </div>
                </CardContent>
              </Card>
        ...
        <ConsoleStatusBar
          ...
          meta={
            lastRefreshedAt
              ? t("admin.status.lastRefreshed", { value: new Date(lastRefreshedAt).toLocaleString() })
              : t("admin.status.waitingForRefresh")
          }
        />
          ...
          <ConsoleSurfaceHeader
            action={
              <div className="flex flex-wrap gap-2">
                <Button className="rounded-xl" disabled={!hasAdminWriteAccess} onClick={openCreateTenantPanel} type="button">
                ...
                <Button className="rounded-xl" disabled={!hasAdminWriteAccess} onClick={openCreateUserPanel} type="button" variant="outline">
                ...
                <Badge
                  ...
                  variant="outline"
                >
                  {hasAdminWriteAccess ? t("admin.access.fullAccess") : t("admin.access.readOnly")}
                </Badge>
              </div>
            }
            description={t("admin.resourceActions.description")}
            title={t("admin.resourceActions.title")}
          />
        </ConsoleSurface>

        <div className="grid gap-6 2xl:grid-cols-[minmax(0,1.28fr)_340px]">
          <div className="space-y-6">
            {showAdvancedAdminSections && adminSection === "overview" ? (
            <ConsoleSurface>
              <ConsoleSurfaceHeader
                description={t("admin.overviewLanes.description")}
              ...
              <div className="grid gap-4 p-6 lg:grid-cols-3">
                {overviewLaneItems.map((item) => (
                  <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-5" key={item.key}>
                    ...
                    <div className="mt-5">
                      <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                        <Link href={item.href}>{t("admin.overviewLanes.openLane")}</Link>
                      </Button>
                    </div>
                  </div>
                ))}
              ...
              <ConsoleSurfaceHeader
                action={
                  <Badge className="border-slate-200 bg-slate-50 text-slate-600" variant="outline">
                    {t("admin.directory.results", { count: String(searchedWorkspaces.length) })}
                  </Badge>
                }
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-slate-100">
                      <TableHead className="px-0">{t("admin.directory.tenant")}</TableHead>
                      <TableHead>{t("admin.directory.workspace")}</TableHead>
                      <TableHead>{t("admin.directory.lifecycle")}</TableHead>
                      <TableHead>{t("admin.directory.knowledgeBases")}</TableHead>
                      <TableHead className="text-right">{t("admin.directory.actionsColumn")}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {searchedWorkspaces.map((workspace) => {
                      const tenant = tenants.find((tenantItem) => tenantItem.id === workspace.tenant_id);
                      const workspaceKnowledgeBaseCount = searchedKnowledgeBases.filter(
                        (knowledgeBase) => knowledgeBase.workspace_id === workspace.id
                      ).length;

                      return (
                        <TableRow className="border-slate-100" key={workspace.id}>
                          <TableCell className="px-0">
                            <div className="font-medium text-slate-900">{tenant?.name ?? t("admin.directory.unknownTenant")}</div>
                            <div className="mt-1 text-xs text-slate-400">{tenant?.slug ?? t("admin.directory.notAvailable")}</div>
                          </TableCell>
                          <TableCell>
                            <div className="font-medium text-slate-800">{workspace.name}</div>
                            <div className="mt-1 text-xs text-slate-400">{workspace.slug}</div>
                          </TableCell>
                          <TableCell>
                            <Badge className={cn("border", getWorkspaceLifecycleClass(workspace.is_archived))} variant="outline">
                              {workspace.is_archived ? t("admin.directory.archived") : t("admin.directory.active")}
                            </Badge>
                          </TableCell>
                          ...
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-2">
                              <Button
                                className="bg-white"
                                disabled={!hasAdminWriteAccess || isUpdatingResource || isCreatingResource}
                                onClick={() => openWorkspaceEditPanel(workspace)}
                                size="sm"
                                type="button"
                                variant="outline"
                              >
                              ...
                              <Button
                                ...
                                onClick={() => void handleToggleWorkspaceLifecycle(workspace)}
                                size="sm"
                                type="button"
                                variant="outline"
                              >
                              ...
                              <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                                <Link href={buildWorkspaceContextHref(workspace, "chat")}>{t("admin.directory.openChat")}</Link>
                              </Button>
                              <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                                <Link href={buildWorkspaceContextHref(workspace, "documents")}>{t("shell.nav.documents")}</Link>
                              </Button>
                              <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                                <Link href={buildWorkspaceContextHref(workspace, "workflows")}>{t("shell.userMenu.operations")}</Link>
                              </Button>
                            </div>
                          </TableCell>
                        </TableRow>
                    ...
                    {searchedWorkspaces.length === 0 && (
                      <TableRow className="border-slate-100">
                        <TableCell className="px-0 py-8 text-slate-500" colSpan={5}>
            ...
            <ConsoleSurface>
              <ConsoleSurfaceHeader
                action={
                  <Badge className="border-slate-200 bg-slate-50 text-slate-600" variant="outline">
                    {t("admin.directory.results", { count: String(searchedKnowledgeBases.length) })}
                  </Badge>
                }
              ...
              <div className="overflow-x-auto px-6 py-4">
                <Table>
                  <TableHeader>
                    <TableRow className="border-slate-100">
                      <TableHead className="px-0">{t("admin.governance.knowledgeBase")}</TableHead>
                      <TableHead>{t("admin.governance.workspace")}</TableHead>
                      <TableHead>{t("admin.governance.publication")}</TableHead>
                      <TableHead>{t("settings.retrievalProfiles.title")}</TableHead>
                      <TableHead>{t("admin.governance.tenant")}</TableHead>
                      <TableHead className="text-right">{t("admin.governance.actionsColumn")}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {searchedKnowledgeBases.map((knowledgeBase) => {
                      ...
                      return (
                        <TableRow className="border-slate-100" key={knowledgeBase.id}>
                          <TableCell className="px-0">
                            <div className="font-medium text-slate-900">{knowledgeBase.name}</div>
                            <div className="mt-1 text-xs text-slate-400">{knowledgeBase.slug}</div>
                          </TableCell>
                          <TableCell>{workspace?.name ?? t("admin.governance.unknownWorkspace")}</TableCell>
                          <TableCell>
                            <Badge className={cn("border", getKnowledgeBasePublicationClass(knowledgeBase.publication_status))} variant="outline">
                              {knowledgeBase.publication_status === "published" ? t("admin.governance.published") : t("admin.governance.draft")}
                            </Badge>
                          </TableCell>
                          <TableCell>
                            {effectiveRetrievalProfile ? (
                              <div className="flex flex-wrap gap-2">
                                <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                                  {t("home.retrievalInspector.retrievalProfile", { value: effectiveRetrievalProfile.name })}
                                </Badge>
                                <Badge className="border-slate-200 bg-slate-50 text-slate-600" variant="outline">
                                  {t("home.retrievalInspector.retrievalMode", {
                                    value: t(`settings.retrievalProfiles.modes.${effectiveRetrievalProfile.retrieval_mode}`)
                                  })}
                                </Badge>
                                {!assignedRetrievalProfile ? (
                              ...
                              <span className="text-sm text-slate-500">{t("settings.governance.empty")}</span>
                            )}
                          </TableCell>
                          <TableCell>{tenant?.name ?? t("admin.governance.unknownTenant")}</TableCell>
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-2">
                              <Button
                                className="bg-white"
                                disabled={!hasAdminWriteAccess || isUpdatingResource || isCreatingResource}
                                onClick={() => openKnowledgeBaseEditPanel(knowledgeBase)}
                                size="sm"
                                type="button"
                                variant="outline"
                              >
                              ...
                              <Button
                                ...
                                disabled={
                                  !hasAdminWriteAccess ||
                                  activeKnowledgeBaseActionId === knowledgeBase.id ||
                                  activeWorkspaceActionId !== null ||
                                  isUpdatingResource ||
                                  isCreatingResource
                                }
                                onClick={() => void handleToggleKnowledgeBasePublication(knowledgeBase)}
                                size="sm"
                                type="button"
                                variant="outline"
                              >
                              ...
                              <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                                <Link href={buildAdminWorkspaceHref("directory", {
                                  view: "chat",
                                  tenantId: knowledgeBase.tenant_id,
                                  workspaceId: knowledgeBase.workspace_id,
                                  knowledgeBaseId: knowledgeBase.id
                                })}>{t("shell.nav.chat")}</Link>
                              </Button>
                              <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                                <Link href={buildKnowledgeBaseContextHref(knowledgeBase, "documents")}>{t("shell.nav.documents")}</Link>
                              </Button>
                              <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                                <Link href={buildKnowledgeBaseContextHref(knowledgeBase, "workflows")}>{t("shell.userMenu.operations")}</Link>
                              </Button>
                            </div>
                          </TableCell>
                        </TableRow>
                    ...
                    {searchedKnowledgeBases.length === 0 && (
                      <TableRow className="border-slate-100">
                        <TableCell className="px-0 py-8 text-slate-500" colSpan={6}>
            ...
            <ConsoleSurface>
              <ConsoleSurfaceHeader
                action={
                  <Badge className="border-slate-200 bg-slate-50 text-slate-600" variant="outline">
                    {t("admin.directory.results", { count: String(searchedAgents.length) })}
                  </Badge>
                }
              ...
              <div className="overflow-x-auto px-6 py-4">
                <Table>
                  <TableHeader>
                    <TableRow className="border-slate-100">
                      <TableHead className="px-0">{t("admin.agents.agent")}</TableHead>
                      ...
                      <TableHead className="text-right">{t("admin.agents.actionsColumn")}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {searchedAgents.map((agent) => {
                      ...
                      return (
                        <TableRow className="border-slate-100" key={agent.id}>
                          ...
                          <TableCell>{tenant?.name ?? t("admin.directory.unknownTenant")}</TableCell>
                          <TableCell>{t(`agents.modes.${agent.mode}`)}</TableCell>
                          <TableCell>
                            <Badge
                              className={cn(
                                "border",
                          ...
                          <TableCell>{agent.knowledge_base_scope || t("admin.agents.unscoped")}</TableCell>
                            ...
                            <div className="flex justify-end gap-2">
                              ...
                              <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                                <Link
                                  href={buildOperationsHref({
                                    tenantId: agent.tenant_id,
                                    status: agent.mode === "workflow_recovery" ? "failed" : "all"
                                  })}
                                >
                    ...
                    {searchedAgents.length === 0 && (
                      <TableRow className="border-slate-100">
                        <TableCell className="px-0 py-8 text-slate-500" colSpan={7}>
            ...
            {adminSection === "access" || (showAdvancedAdminSections && adminSection === "security") ? (
            <ConsoleSurface>
              <ConsoleSurfaceHeader
                action={
                  <div className="flex flex-wrap items-center gap-2">
                    <Select disabled={isLoading} onValueChange={setMemberRelationshipFilter} value={memberRelationshipFilter}>
                    ...
                    <Select disabled={isLoading} onValueChange={setMemberAccountFilter} value={memberAccountFilter}>
                    ...
                    <Badge className="border-slate-200 bg-slate-50 text-slate-600" variant="outline">
                      {t("admin.directory.results", { count: String(searchedUsers.length) })}
                    </Badge>
                  </div>
                }
              ...
              <div className="overflow-x-auto px-6 py-4">
                <Table>
                  <TableHeader>
                    <TableRow className="border-slate-100">
                      <TableHead className="px-0">{t("admin.members.member")}</TableHead>
                      <TableHead>{t("admin.members.memberships")}</TableHead>
                      <TableHead>{t("admin.members.account")}</TableHead>
                      <TableHead className="text-right">{t("admin.members.actionsColumn")}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {searchedUsers.map((user) => {
                      const scopedMembership =
                        selectedTenantId === "all"
                          ? null
                          : user.memberships.find((membership) => membership.tenant_id === selectedTenantId) ?? null;
                      const scopedInvitationCredential = scopedMembership
                        ? revealedInvitationByMembershipId[scopedMembership.id] ?? null
                        : null;
                      const invitationExpired = isInvitationExpired(
                        scopedInvitationCredential?.invitation_expires_at ?? scopedMembership?.invitation_expires_at ?? null
                      );
                        ...
                        <TableRow className="border-slate-100" key={user.id}>
                          ...
                          <TableCell>
                            <div className="space-y-3">
                              ...
                              {scopedMembership?.membership_status === "invited" && scopedInvitationCredential ? (
                                <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-3 text-left">
                                  ...
                                  <div className="mt-1 text-xs text-amber-800">
                                    {t("admin.members.invitationIssuedAt", {
                                      value: scopedInvitationCredential.invited_at
                                        ? formatTimestamp(scopedInvitationCredential.invited_at)
                                  ...
                                  <div className="mt-1 text-xs text-amber-800">
                                    {t("admin.members.invitationIssueCount", {
                                      value: String(scopedInvitationCredential.invitation_issue_count),
                                    })}
                                  </div>
                                  <div className="mt-1 text-xs text-amber-800">
                                    {t("admin.members.invitationExpiresAt", {
                                      value: scopedInvitationCredential.invitation_expires_at
                                        ? formatTimestamp(scopedInvitationCredential.invitation_expires_at)
                                        : t("admin.directory.notAvailable"),
                                    })}
                                  </div>
                                </div>
                          ...
                          <TableCell>
                            <Badge
                              ...
                              variant="outline"
                            >
                              {user.is_active ? t("admin.members.activeAccount") : t("admin.members.inactiveAccount")}
                            </Badge>
                            <div className="mt-2 text-xs text-slate-500">
                              {t("admin.members.createdAt", { value: formatTimestamp(user.created_at) })}
                            </div>
                          ...
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-2">
                              ...
                              <Button
                                className="bg-white"
                                disabled={!hasAdminWriteAccess || activeUserActionId === user.id || isLoading}
                                onClick={() => void handleToggleUserAccount(user)}
                                size="sm"
                                type="button"
                                variant="outline"
                              >
                              ...
                              {selectedTenantId === "all" ? (
                                <Button className="bg-white" disabled size="sm" type="button" variant="outline">
                                ...
                                <Button
                                  className="bg-white"
                                  disabled={!hasAdminWriteAccess || activeMembershipActionId === scopedMembership.id || isLoading}
                                  onClick={() => void handleIssueMembershipInvitation(user, scopedMembership)}
                                  size="sm"
                                  type="button"
                                  variant="outline"
                                >
                                  {activeMembershipActionId === scopedMembership.id
                                ...
                                {scopedMembership.membership_status === "invited" ? (
                                  <Button
                                    className="bg-white"
                                    disabled={!hasAdminWriteAccess || activeMembershipActionId === scopedMembership.id || isLoading}
                                    onClick={() => void handleRevokeMembershipInvitation(user, scopedMembership)}
                                    size="sm"
                                    type="button"
                                    variant="outline"
                                  >
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    void handleUpdateMembership(
                                        user,
                                        scopedMembership,
                                        scopedMembership.membership_status === "active" ? "suspended" : "active"
                                      )
                                    }
                                  size="sm"
                                  type="button"
                                ...
                                >
                                    {activeMembershipActionId === scopedMembership.id
                                      ? scopedMembership.membership_status === "active"
                                  ...
                                  <Button
                                    className="bg-white"
                                    disabled={!hasAdminWriteAccess || activeMembershipActionId === scopedMembership.id || isLoading}
                                    onClick={() => void handleRemoveMembership(user, scopedMembership)}
                                    size="sm"
                                    type="button"
                                    variant="outline"
                                  >
                                    {activeMembershipActionId === scopedMembership.id ? t("admin.actions.removing") : t("admin.actions.removeFromTenant")}
                                  </Button>
                                </>
                              ) : (
                                <Button
                                  className="bg-white"
                                  disabled={!hasAdminWriteAccess || activeUserActionId === user.id || isLoading}
                                  onClick={() => void handleAddUserToTenant(user)}
                                  size="sm"
                                  type="button"
                                  variant="outline"
                                >
                                  {activeUserActionId === user.id ? t("admin.actions.invitingToTenant") : t("admin.actions.inviteToTenant")}
                                </Button>
                              )}
                            </div>
                          </TableCell>
                    ...
                    {searchedUsers.length === 0 ? (
                      <TableRow className="border-slate-100">
                        <TableCell className="px-0 py-8 text-slate-500" colSpan={4}>
          ...
          <div className="space-y-6">
            ...
            <ConsoleSurface>
              <ConsoleSurfaceHeader
                action={
                  <Select disabled={isLoading} onValueChange={setAuditEventFilter} value={auditEventFilter}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="sign_in_succeeded">{t("admin.audit.eventTypes.signInSucceeded")}</SelectItem>
                      <SelectItem value="invitation_issued">{t("admin.audit.eventTypes.invitationIssued")}</SelectItem>
                      <SelectItem value="invitation_activated">{t("admin.audit.eventTypes.invitationActivated")}</SelectItem>
                      <SelectItem value="invitation_revoked">{t("admin.audit.eventTypes.invitationRevoked")}</SelectItem>
                      <SelectItem value="membership_active">{t("admin.audit.eventTypes.membershipActive")}</SelectItem>
                      <SelectItem value="membership_suspended">{t("admin.audit.eventTypes.membershipSuspended")}</SelectItem>
                      <SelectItem value="membership_deleted">{t("admin.audit.eventTypes.membershipDeleted")}</SelectItem>
                    </SelectContent>
                  </Select>
                }
              ...
              <div className="space-y-3 px-6 py-5">
                {auditEvents.length > 0 ? (
                  auditEvents.map((event) => (
                    <div className="rounded-[18px] border border-slate-100 bg-slate-50/70 p-4" key={event.id}>
                      ...
                      <div className="mt-2 text-sm text-slate-600">
                        {event.user_display_name ?? t("admin.directory.notAvailable")}
                        {event.tenant_name ? ` · ${event.tenant_name}` : ""}
                      </div>
                      <div className="mt-1 text-xs text-slate-500">
                        {t("admin.audit.actor", {
                          value: event.actor_display_name ?? t("admin.directory.notAvailable"),
                        })}
                      </div>
                      {readAccessEventReason(event) ? (
                        <div className="mt-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600">
                          <span className="font-semibold text-slate-700">{t("admin.audit.reasonLabel")}: </span>
                          {readAccessEventReason(event)}
                        </div>
                      ) : null}
                          ...
                          <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                            <Link
                              href={buildMemberGovernanceHref(event.user_id, {
                                  ...
                                  memberRelationshipFilter:
                                    event.event_type === "invitation_issued" || event.event_type === "invitation_revoked"
                                      ? "invited"
                                      : null
                                })}
                            >
            ...
            <ConsoleSurface>
              ...
              <div className="grid gap-3 p-6">
                {accessSummaryItems.map((item) => (
                  <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4" key={item.label}>
              ...
              <div className="space-y-3 px-6 py-5">
                {invitedActivationQueue.length > 0 ? (
                  invitedActivationQueue.map((item) => (
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-5" key={item.membership.id}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <div className="text-base font-semibold text-slate-950">{item.user.display_name}</div>
                        ...
                        <div className="flex flex-wrap justify-end gap-2">
                          ...
                          <Badge
                            ...
                            variant="outline"
                          >
                            {item.invitationExpired ? t("admin.members.invitationExpired") : t("admin.members.invitedMembership")}
                          </Badge>
                        </div>
                      </div>
                      <div className="mt-3 grid gap-3 md:grid-cols-3">
                        <div className="rounded-xl border border-slate-200 bg-white px-4 py-3">
                          ...
                          <div className="mt-2 font-mono text-sm font-semibold text-slate-900">
                            {item.invitationCredential?.invitation_token ?? t("admin.members.activationQueue.notIssued")}
                          </div>
                        </div>
                        <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600">
                          <div>{t("admin.members.invitationIssuedAt", {
                            value: item.invitationIssuedAt
                              ? formatTimestamp(item.invitationIssuedAt)
                              : t("admin.directory.notAvailable")
                          })}</div>
                          <div className="mt-1">{t("admin.members.invitationIssueCount", {
                            value: String(item.invitationCredential?.invitation_issue_count ?? item.membership.invitation_issue_count)
                          })}</div>
                        </div>
                        <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600">
                          <div>{t("admin.members.invitationExpiresAt", {
                            value: item.invitationExpiresAt
                              ? formatTimestamp(item.invitationExpiresAt)
                              : t("admin.directory.notAvailable")
                          })}</div>
                          <div className="mt-1">{t("admin.members.invitationIssuedBy", {
                            value:
                              item.invitationCredential?.last_invitation_issued_by_display_name ??
                              item.membership.last_invitation_issued_by_display_name ??
                              t("admin.directory.notAvailable")
                          })}</div>
                        </div>
                      </div>
                      <div className="mt-4 flex flex-wrap gap-2">
                        ...
                        <Button
                          className="bg-white"
                          disabled={!hasAdminWriteAccess || activeMembershipActionId === item.membership.id || isLoading}
                          onClick={() => void handleIssueMembershipInvitation(item.user, item.membership)}
                          size="sm"
                          type="button"
                          variant="outline"
                        >
                        ...
                        <Button
                          className="bg-white"
                          disabled={!hasAdminWriteAccess || activeMembershipActionId === item.membership.id || isLoading}
                          onClick={() => void handleUpdateMembership(item.user, item.membership, "active")}
                          size="sm"
                          type="button"
                          variant="outline"
                        >
                        ...
                        <Button
                          className="bg-white"
                          disabled={!hasAdminWriteAccess || activeMembershipActionId === item.membership.id || isLoading}
                          onClick={() => void handleRevokeMembershipInvitation(item.user, item.membership)}
                          size="sm"
                          type="button"
                          variant="outline"
                        >
                ...
                {currentActorSecurityPosture ? (
                  <>
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-5">
                      <div className="text-base font-semibold text-slate-950">{currentActorSecurityPosture.currentActor.display_name}</div>
                      <div className="mt-1 text-sm text-slate-500">{currentActorSecurityPosture.currentActor.email}</div>
                      <div className="mt-3 flex flex-wrap gap-2">
                        <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                          {getRoleLabel(currentActorSecurityPosture.currentActor.role, t)}
                        </Badge>
                        <Badge
                          className={cn(
                            "border",
                            currentActorSecurityPosture.currentActor.is_active
                              ? "border-emerald-200 bg-emerald-50 text-emerald-700"
                              : "border-rose-200 bg-rose-50 text-rose-700"
                          )}
                          variant="outline"
                      ...
                      <div className="mt-4 text-sm text-slate-500">
                        {t("admin.currentActorSecurity.lastSignedIn", {
                          value: currentActorSecurityPosture.currentActor.last_signed_in_at
                            ? formatTimestamp(currentActorSecurityPosture.currentActor.last_signed_in_at)
                            : t("admin.directory.notAvailable"),
                        })}
                      </div>
                      {hasAdminWriteAccess ? (
                        <div className="mt-4">
                          <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                            <Link href={buildMemberGovernanceHref(currentActorSecurityPosture.currentActor)}>
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      <div className="rounded-[18px] border border-slate-100 bg-slate-50/70 p-4">
                        <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.currentActorSecurity.activeMemberships")}</div>
                        <div className="mt-3 text-2xl font-semibold text-slate-950">{currentActorSecurityPosture.activeMembershipCount}</div>
                      </div>
                      <div className="rounded-[18px] border border-slate-100 bg-slate-50/70 p-4">
                        <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.currentActorSecurity.invitedMemberships")}</div>
                        <div className="mt-3 text-2xl font-semibold text-slate-950">{currentActorSecurityPosture.invitedMembershipCount}</div>
                      </div>
                      <div className="rounded-[18px] border border-slate-100 bg-slate-50/70 p-4">
                        <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.currentActorSecurity.suspendedMemberships")}</div>
                        <div className="mt-3 text-2xl font-semibold text-slate-950">{currentActorSecurityPosture.suspendedMembershipCount}</div>
                      </div>
                      <div className="rounded-[18px] border border-slate-100 bg-slate-50/70 p-4">
                        <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.currentActorSecurity.invitationRisk")}</div>
                        <div className="mt-3 text-2xl font-semibold text-slate-950">
                          {currentActorSecurityPosture.expiringInvitationCount + currentActorSecurityPosture.expiredInvitationCount}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                        <Link href="/settings">{t("admin.currentActorSecurity.openSettings")}</Link>
                      </Button>
                      <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                        <Link href={buildAdminHref({ tenantId: selectedTenantId, section: "access" })}>
            ...
            <ConsoleSurface>
              ...
              <div className="space-y-4 px-6 py-5">
                {securityWatchItems.map((item) => (
                  <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-5" key={item.title}>
                    <div className="flex items-center justify-between gap-3">
                      <div className="text-base font-semibold text-slate-900">{item.title}</div>
                      <Badge className={cn("border", getWatchStatusClass(item.status))} variant="outline">
                        {item.status === "attention" ? t("admin.watchlist.attention") : item.status === "review" ? t("admin.watchlist.review") : t("admin.watchlist.healthy")}
                      </Badge>
                    </div>
            ...
            <ConsoleSurface>
              ...
              <div className="space-y-3 px-6 py-5 text-sm text-slate-600">
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.scopePanel.tenantScope")}</div>
                  <div className="mt-2 text-base font-semibold text-slate-900">
                    {selectedTenantId === "all"
                      ? t("admin.scopePanel.allTenants")
                      : tenants.find((tenant) => tenant.id === selectedTenantId)?.name ?? t("admin.scopePanel.unknownTenant")}
                  </div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.scopePanel.visibleInventory")}</div>
                  <div className="mt-2 text-base font-semibold text-slate-900">
                ...
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.scopePanel.searchResults")}</div>
                  <div className="mt-2 text-base font-semibold text-slate-900">
                    {t("admin.scopePanel.inventoryCounts", {
                  ...
                  <div className="mt-1 text-sm text-slate-500">
                    {normalizedSearchQuery ? t("admin.scopePanel.query", { value: searchQuery.trim() }) : t("admin.scopePanel.noDirectorySearch")}
                  </div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.scopePanel.lifecycleFilters")}</div>
                  <div className="mt-2 text-base font-semibold text-slate-900">
                ...
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("settings.retrievalProfiles.title")}</div>
                  <div className="mt-2 text-base font-semibold text-slate-900">
                    {retrievalProfileFilter === "all"
                      ...
                      : retrievalProfileFilter === DEFAULT_RETRIEVAL_PROFILE_FILTER_VALUE
                        ...
                        : retrievalProfileFilter === DISABLED_RETRIEVAL_PROFILE_FILTER_VALUE
                          ...
                          : retrievalProfiles.find((retrievalProfile) => retrievalProfile.id === retrievalProfileFilter)?.name ??
                ...
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.scopePanel.primaryRoute")}</div>
                  <div className="mt-2 text-base font-semibold text-slate-900">
                    {scopedPrimaryWorkspace?.name ?? t("admin.scopePanel.noScopedWorkspace")}
                  </div>
                  <div className="mt-1 text-sm text-slate-500">
                    {scopedPrimaryKnowledgeBase?.name ?? t("admin.scopePanel.noScopedKnowledgeBase")}
                  </div>
                  <div className="mt-4 flex flex-wrap gap-2">
                    <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                    ...
                    <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                      <Link href={buildAdminWorkspaceHref(adminSection, {
                        view: "documents",
                        tenantId: selectedTenantId === "all" ? scopedPrimaryWorkspace?.tenant_id ?? null : selectedTenantId,
                        workspaceId: scopedPrimaryWorkspace?.id ?? null,
                        knowledgeBaseId: scopedPrimaryKnowledgeBase?.id ?? null
                      })}>{t("shell.nav.documents")}</Link>
                    </Button>
                    <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                      <Link href={buildAdminWorkspaceHref(adminSection, {
                        view: "workflows",
                        ...
                        workspaceId: scopedPrimaryWorkspace?.id ?? null,
                        knowledgeBaseId: scopedPrimaryKnowledgeBase?.id ?? null
                      })}>{t("shell.userMenu.operations")}</Link>
                    </Button>
                    <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                      <Link href={buildAdminWorkspaceHref(adminSection, {
                        view: "documents",
                        ...
                        knowledgeBaseId: scopedPrimaryKnowledgeBase?.id ?? null,
                        documentStatus: "failed"
                      })}>{t("admin.scopePanel.failedDocuments")}</Link>
                    </Button>
                    <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                      <Link href={buildAdminWorkspaceHref(adminSection, {
                        view: "workflows",
                        ...
                        knowledgeBaseId: scopedPrimaryKnowledgeBase?.id ?? null,
                        workflowStatus: "failed"
                      })}>{t("admin.scopePanel.failedWorkflows")}</Link>
                    </Button>
                  </div>
                </div>
              </div>
            ...
            <ConsoleSurface>
              ...
              <div className="p-6">
                <ConsoleRuntimeTaskPacket
                  ...
                  primaryActionLabel={t("admin.runtimeTaskPacket.primaryAction")}
                  primaryActionRunRecord={runtimeTaskPacket.primaryActionRunRecord}
            ...
            <ConsoleSurface>
              <ConsoleSurfaceHeader
                action={
                  <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                    <Link href={buildOperationsHref({ tenantId: selectedTenantId === "all" ? null : selectedTenantId, lane: "overview", status: "all" })}>
              ...
              <div className="grid gap-4 p-6 md:grid-cols-3">
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.agentExecutions.metrics.total")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{scopedAgentExecutionMetrics.total_executions}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.agentExecutions.metrics.totalHint")}</div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.agentExecutions.metrics.completed")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{scopedAgentExecutionMetrics.completed_executions}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.agentExecutions.metrics.completedHint")}</div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.agentExecutions.metrics.failed")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{scopedAgentExecutionMetrics.failed_executions}</div>
                  <div className="mt-1 text-sm text-slate-500">
                    {scopedAgentExecutionMetrics.latest_execution_at
                      ? t("admin.agentExecutions.latestExecution", {
                          value: formatTimestamp(scopedAgentExecutionMetrics.latest_execution_at)
                        })
              ...
              <div className="grid gap-4 border-t border-slate-100 p-6 xl:grid-cols-2">
                <div className="space-y-3">
                  <div className="text-sm font-semibold text-slate-950">{t("admin.agentExecutions.tenantBreakdown")}</div>
                  {tenantAgentExecutionActivity.slice(0, 4).map((item) => (
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4" key={item.tenant.id}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <div className="text-base font-semibold text-slate-900">{item.tenant.name}</div>
                          <div className="mt-1 text-sm text-slate-500">{item.tenant.slug}</div>
                        </div>
                        <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                          {t("admin.agentExecutions.tenantExecutionCount", { count: String(item.metrics.total_executions) })}
                        </Badge>
                      </div>
                      <div className="mt-3 flex flex-wrap gap-2">
                        <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                          {t("admin.agentExecutions.tenantCompletedCount", { count: String(item.metrics.completed_executions) })}
                        </Badge>
                        <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                          {t("admin.agentExecutions.tenantFailedCount", { count: String(item.metrics.failed_executions) })}
                        </Badge>
                      </div>
                      <div className="mt-3 text-sm text-slate-500">
                        {item.metrics.latest_execution_at
                          ? t("admin.agentExecutions.latestExecution", { value: formatTimestamp(item.metrics.latest_execution_at) })
                          : t("admin.agentExecutions.noLatestExecution")}
                      </div>
                      <div className="mt-4 flex flex-wrap gap-2">
                        <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                          <Link href={item.operationsHref}>{t("admin.agentExecutions.openOperations")}</Link>
                        </Button>
                        <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                          <Link href={item.agentsHref}>{t("admin.agentExecutions.openAgents")}</Link>
                        </Button>
                      </div>
                    </div>
                  ))}
                ...
                <div className="space-y-3">
                  <div className="text-sm font-semibold text-slate-950">{t("admin.agentExecutions.recentExecutions")}</div>
                  {recentScopedAgentExecutions.length > 0 ? (
                  recentScopedAgentExecutions.map((execution) => {
                      const linkedAgent = agents.find((agent) => agent.id === execution.agent_definition_id) ?? null;
                      ...
                      const followUpActions = buildAgentExecutionFollowUpActions({
                        sourceContext: { surface: "admin", section: adminSection },
                        execution,
                      ...
                      return (
                        <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4" key={execution.id}>
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              <div className="text-sm font-semibold text-slate-950">
                                {linkedAgent?.name ?? t("admin.agentRuntime.unknownAgent")}
                              </div>
                            ...
                            <div className="mt-4 rounded-xl border border-slate-200 bg-white px-4 py-3">
                              ...
                              <div className="mt-3 space-y-2">
                                {evidenceSummary.retrievalSources.map((source, index) => (
                                  <div className="rounded-lg border border-slate-100 bg-slate-50/80 px-3 py-3" key={`${execution.id}-${source.documentChunkId ?? index}`}>
                                    <div className="flex flex-wrap items-center gap-2">
                          ...
                          {recommendedActions.length > 0 ? (
                            <div className="mt-4 flex flex-wrap gap-2">
                              {recommendedActions.map((action) => (
                                <Badge className="border-slate-200 bg-white text-slate-700" key={action} variant="outline">
                                  {action}
                                </Badge>
                              ))}
                            </div>
                            ...
                            {followUpActions.length > 0 ? (
                              <>
                                ...
                                <div className="mt-3 flex flex-wrap gap-2">
                                  ...
                                  <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                                    <Link href={executionHref}>{t("admin.agentExecutions.openExecutionRoute")}</Link>
                                  </Button>
                                </div>
                              </>
                            ) : (
            ...
            <ConsoleSurface>
              ...
              <div className="grid gap-4 p-6 md:grid-cols-2 xl:grid-cols-4">
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.runtimeGovernance.metrics.attentionAgents")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{governancePosture.attentionAgents}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.runtimeGovernance.metrics.attentionAgentsHint")}</div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.runtimeGovernance.metrics.disabledBoundModels")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{governancePosture.disabledBoundModels}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.runtimeGovernance.metrics.disabledBoundModelsHint")}</div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.runtimeGovernance.metrics.disabledBoundTools")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{governancePosture.disabledBoundTools}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.runtimeGovernance.metrics.disabledBoundToolsHint")}</div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.runtimeGovernance.metrics.approvalGatedTools")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{governancePosture.approvalGatedTools}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.runtimeGovernance.metrics.approvalGatedToolsHint")}</div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.runtimeGovernance.metrics.disabledBoundRetrievalProfiles")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{retrievalGovernanceSummary.disabledBoundRetrievalProfiles}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.runtimeGovernance.metrics.disabledBoundRetrievalProfilesHint")}</div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.runtimeGovernance.metrics.defaultFallbackKnowledgeBases")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{retrievalGovernanceSummary.defaultFallbackKnowledgeBases}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.runtimeGovernance.metrics.defaultFallbackKnowledgeBasesHint")}</div>
                </div>
              </div>
              <div className="grid gap-4 border-t border-slate-100 p-6 xl:grid-cols-2">
                <div className="space-y-3">
                  <div className="text-sm font-semibold text-slate-950">{t("admin.runtimeGovernance.signals")}</div>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                      <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.runtimeGovernance.activeAgentsWithoutScope")}</div>
                      <div className="mt-2 text-2xl font-semibold text-slate-950">{governancePosture.activeAgentsWithoutScope}</div>
                    </div>
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                      <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.runtimeGovernance.missingModelBindings")}</div>
                      <div className="mt-2 text-2xl font-semibold text-slate-950">{governancePosture.agentsMissingModel}</div>
                    </div>
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                      <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.runtimeGovernance.disabledModelBindings")}</div>
                      <div className="mt-2 text-2xl font-semibold text-slate-950">{governancePosture.agentsUsingDisabledModel}</div>
                    </div>
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                      <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.runtimeGovernance.disabledToolBindings")}</div>
                      <div className="mt-2 text-2xl font-semibold text-slate-950">{governancePosture.agentsUsingDisabledToolRegistration}</div>
                    </div>
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                      <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.runtimeGovernance.missingRetrievalBindings")}</div>
                      <div className="mt-2 text-2xl font-semibold text-slate-950">{governancePosture.agentsMissingRetrievalProfile}</div>
                    </div>
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                      <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.runtimeGovernance.disabledRetrievalBindings")}</div>
                      <div className="mt-2 text-2xl font-semibold text-slate-950">{governancePosture.agentsUsingDisabledRetrievalProfile}</div>
                    </div>
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                      <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.runtimeGovernance.disabledRetrievalAssignments")}</div>
                      <div className="mt-2 text-2xl font-semibold text-slate-950">{retrievalGovernanceSummary.knowledgeBasesUsingDisabledRetrievalProfile}</div>
                    </div>
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                      <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.runtimeGovernance.explicitRetrievalBindings")}</div>
                      <div className="mt-2 text-2xl font-semibold text-slate-950">{retrievalGovernanceSummary.explicitAssignedKnowledgeBases}</div>
                    </div>
                  </div>
                </div>

                <div className="space-y-3">
                  <div className="text-sm font-semibold text-slate-950">{t("admin.runtimeGovernance.quickActions")}</div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                      <Link href={focusedRuntimeSettingsHref}>{t("admin.runtimeGovernance.openSettings")}</Link>
                    </Button>
                    <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                      <Link href={buildAdminHref({
                        tenantId: selectedTenantId === "all" ? null : selectedTenantId,
                        section: "directory",
                        retrievalProfileFilter: DEFAULT_RETRIEVAL_PROFILE_FILTER_VALUE
                      })}>
                        {t("admin.runtimeGovernance.openDefaultFallbackKnowledgeBases")}
                      </Link>
                    </Button>
                    <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                    ...
                    <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                      <Link href={buildAdminHref({
                        ...
                        {t("admin.runtimeGovernance.openDisabledRetrievalAssignments")}
                      </Link>
                    </Button>
                  </div>
                  <div className="pt-1 text-sm font-semibold text-slate-950">{t("admin.runtimeGovernance.issueActions")}</div>
                  <div className="flex flex-wrap gap-2">
                    <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                      <Link
                        href={buildAgentsHref({
                    ...
                    {focusedApprovalToolDefinitionsHref ? (
                      <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                        <Link href={focusedApprovalToolDefinitionsHref}>{t("agents.readiness.issueLabels.tool_approval_required")}</Link>
                      </Button>
                    ) : null}
                    ...
                    {governancePosture.issue_counts.tool_mcp_integration_pending > 0 ? (
                      <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                        <Link
                          href={buildAgentsHref({
                            tenantId: selectedTenantId === "all" ? null : selectedTenantId,
              ...
              <div className="grid gap-4 px-6 py-5 xl:grid-cols-2">
                {retrievalProfileAssets.length > 0 ? (
                  retrievalProfileAssets.map((asset) => (
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-5" key={asset.profile.id}>
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          <div className="text-base font-semibold text-slate-950">{asset.profile.name}</div>
                          <div className="mt-1 text-sm text-slate-500">{asset.profile.slug}</div>
                        </div>
                        <Badge className={cn("border", getWatchStatusClass(asset.status))} variant="outline">
                          {asset.status === "attention" ? t("admin.watchlist.attention") : asset.status === "review" ? t("admin.watchlist.review") : t("admin.watchlist.healthy")}
                        </Badge>
                      </div>
                      <div className="mt-3 flex flex-wrap gap-2">
                        <Badge className={cn("border", getRetrievalMethodBadgeClassName(asset.profile.retrieval_mode))} variant="outline">
                          {t(`settings.retrievalProfiles.modes.${asset.profile.retrieval_mode}`)}
                        </Badge>
                        ...
                        <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                          {asset.profile.is_enabled ? t("settings.runtime.enabled") : t("settings.runtime.disabled")}
                        </Badge>
                        <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                          {t("home.retrievalInspector.effectiveTopK", { value: String(asset.profile.top_k) })}
                        </Badge>
                      </div>
                      <div className="mt-4 grid gap-3 sm:grid-cols-2">
                        <div className="rounded-2xl border border-slate-100 bg-white px-4 py-3">
                          <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-400">
                            {t("admin.retrievalProfiles.metrics.assignedKnowledgeBases")}
                          </div>
                          <div className="mt-2 text-2xl font-semibold text-slate-950">{asset.assignedKnowledgeBases}</div>
                          <div className="mt-1 text-sm text-slate-500">
                            {t("admin.retrievalProfiles.metrics.workspaceCoverage", { count: String(asset.assignedWorkspaceCount) })}
                          </div>
                        </div>
                        <div className="rounded-2xl border border-slate-100 bg-white px-4 py-3">
                          <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-400">
                            {t("admin.retrievalProfiles.metrics.defaultFallbackCoverage")}
                          </div>
                          <div className="mt-2 text-2xl font-semibold text-slate-950">{asset.fallbackCoverageCount}</div>
                          <div className="mt-1 text-sm text-slate-500">
                            {t("admin.retrievalProfiles.metrics.publicationMix", {
                              published: String(asset.publishedKnowledgeBaseCount),
                      ...
                      <div className="mt-4 rounded-2xl border border-slate-100 bg-white px-4 py-3 text-sm leading-6 text-slate-600">
                        {!asset.profile.is_enabled && asset.assignedKnowledgeBases > 0
                          ? t("admin.retrievalProfiles.status.disabledAssigned", { count: String(asset.assignedKnowledgeBases) })
                          : asset.fallbackCoverageCount > 0
                            ? t("admin.retrievalProfiles.status.defaultFallback", { count: String(asset.fallbackCoverageCount) })
                            : asset.assignedKnowledgeBases === 0
                              ? t("admin.retrievalProfiles.status.unused")
                              : t("admin.retrievalProfiles.status.healthyAssigned", { count: String(asset.assignedKnowledgeBases) })}
                      </div>
                      <div className="mt-4 flex flex-wrap gap-2">
                        <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                          <Link
                        ...
                        <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                          <Link href={buildSettingsHref({ runtimeResource: "retrieval_profile", retrievalProfileId: asset.profile.id })}>
              ...
              <div className="space-y-3 px-6 py-5">
                {runtimeRoutes.length > 0 ? (
                  runtimeRoutes.map((item) => (
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4" key={item.agent.id}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <div className="text-base font-semibold text-slate-900">{item.agent.name}</div>
                          <div className="mt-1 text-sm text-slate-500">{t(`agents.modes.${item.agent.mode}`)}</div>
                        </div>
                        <Badge
                          className={cn(
                            "border",
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                          {t("admin.runtimeRoutes.scopeLabel", {
                            value: item.scopeLabel?.trim().length ? item.scopeLabel : t("admin.runtimeRoutes.scopeUnbound")
                          })}
                        </Badge>
                        {item.resolvedWorkspace ? (
                          <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                            {t("admin.runtimeRoutes.resolvedWorkspace", { value: item.resolvedWorkspace.name })}
                          </Badge>
                        ) : null}
                        {item.resolvedKnowledgeBase ? (
                          <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                            {t("admin.runtimeRoutes.resolvedKnowledgeBase", { value: item.resolvedKnowledgeBase.name })}
                          </Badge>
                        ) : null}
                        {item.retrievalProfile ? (
                          <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                            {t("admin.runtimeRoutes.retrievalProfile", { value: item.retrievalProfile.name })}
                          </Badge>
                        ) : null}
                      </div>
                      {item.retrievalIssue ? (
                        <div className="mt-3 rounded-[16px] border border-amber-200 bg-amber-50/80 px-4 py-3 text-sm leading-6 text-amber-800">
                          {item.retrievalIssue === "disabled"
                            ? t("admin.runtimeRoutes.retrievalDisabledDetail", {
                                profile: item.retrievalProfile?.name ?? t("agents.dependencies.noRetrievalProfile")
                              })
                      ...
                      <div className="mt-3 rounded-[16px] border border-slate-200 bg-white px-4 py-3">
                        ...
                        <div className="mt-2 text-sm leading-6 text-slate-700">{item.prompt}</div>
                      </div>
                      <div className="mt-4 flex flex-wrap gap-2">
                        <AgentRunButtonLink href={item.recommendedHref} runRecord={item.recommendedRunRecord} size="sm" type="button">
                        ...
                        {item.retrievalIssue === "disabled" ? (
                          <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                            <Link
                              href={
                                item.resolvedKnowledgeBase
                                  ? buildKnowledgeBaseGovernanceHref(item.resolvedKnowledgeBase, {
                                      retrievalProfileFilter: DISABLED_RETRIEVAL_PROFILE_FILTER_VALUE
                                    })
                                  : buildAdminHref({
            ...
            <ConsoleSurface>
              <ConsoleSurfaceHeader
                action={
                  <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                    <Link href={buildAgentsHref({ tenantId: selectedTenantId === "all" ? null : selectedTenantId, status: "active" })}>{t("admin.agentRuntime.openAgents")}</Link>
                  </Button>
                }
              ...
              <div className="grid gap-4 p-6 md:grid-cols-2 xl:grid-cols-4">
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("agents.runs.metrics.total")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{scopedAgentRunMetrics.total_runs}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.agentRuntime.metrics.totalHint")}</div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("agents.runs.metrics.chat")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{scopedAgentRunMetrics.chat_runs}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.agentRuntime.metrics.chatHint")}</div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("agents.runs.metrics.documents")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{scopedAgentRunMetrics.document_runs}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.agentRuntime.metrics.documentsHint")}</div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("agents.runs.metrics.operations")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{scopedAgentRunMetrics.operations_runs}</div>
                  <div className="mt-1 text-sm text-slate-500">
                    {scopedAgentRunMetrics.latest_launched_at
                      ? t("admin.agentRuntime.latestLaunch", { value: formatTimestamp(scopedAgentRunMetrics.latest_launched_at) })
              ...
              <div className="grid gap-4 border-t border-slate-100 p-6 xl:grid-cols-2">
                <div className="space-y-3">
                  <div className="text-sm font-semibold text-slate-950">{t("admin.agentRuntime.tenantBreakdown")}</div>
                  {tenantAgentRuntimeActivity.slice(0, 4).map((item) => (
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4" key={item.tenant.id}>
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                          {t("admin.agentRuntime.tenantRunCount", { count: String(item.metrics.total_runs) })}
                        </Badge>
                      </div>
                      <div className="mt-3 flex flex-wrap gap-2">
                        <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                          {t("agents.runs.metrics.chat")}: {item.metrics.chat_runs}
                        </Badge>
                        <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                          {t("agents.runs.metrics.documents")}: {item.metrics.document_runs}
                        </Badge>
                        <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                          {t("agents.runs.metrics.operations")}: {item.metrics.operations_runs}
                        </Badge>
                      </div>
                      <div className="mt-3 text-sm text-slate-500">
                        {item.metrics.latest_launched_at
                          ? t("admin.agentRuntime.latestLaunch", { value: formatTimestamp(item.metrics.latest_launched_at) })
                          : t("admin.agentRuntime.noLatestLaunch")}
                      </div>
                      <div className="mt-4 flex flex-wrap gap-2">
                        <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                          <Link href={item.agentsHref}>{t("admin.agentRuntime.openAgents")}</Link>
                        </Button>
                        <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                          <Link href={item.operationsHref}>{t("shell.userMenu.operations")}</Link>
                        </Button>
                      </div>
                    </div>
                  ))}
                ...
                <div className="space-y-3">
                  <div className="text-sm font-semibold text-slate-950">{t("admin.agentRuntime.recentRuns")}</div>
                  {recentScopedAgentRuns.length > 0 ? (
                    recentScopedAgentRuns.map((agentRun) => {
                      const linkedAgent = agents.find((agent) => agent.id === agentRun.agent_definition_id) ?? null;

                      return (
                        <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4" key={agentRun.id}>
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              <div className="text-sm font-semibold text-slate-950">
                                {linkedAgent?.name ?? t("admin.agentRuntime.unknownAgent")}
                          ...
                          <div className="mt-3 flex flex-wrap gap-2">
                            ...
                            <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                              {formatAgentRunTriggerSourceLabel(agentRun.trigger_source, t)}
                            </Badge>
                            <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                          ...
                          {agentRun.navigation_href ? (
                            <div className="mt-4">
                              <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                                <a href={agentRun.navigation_href}>{t("agents.runs.openRoute")}</a>
                              </Button>
                            </div>
                          ) : null}
                        </div>
            ...
            <ConsoleSurface>
              <ConsoleSurfaceHeader
                action={
                  <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                    <Link href={workflowSignals.highestPressureTenant?.overviewHref ?? buildOperationsHref({ tenantId: selectedTenantId === "all" ? null : selectedTenantId, lane: "overview", status: "all" })}>
              ...
              <div className="grid gap-4 p-6 md:grid-cols-2 xl:grid-cols-4">
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.workflowRuntime.metrics.failed")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{scopedWorkflowMetrics.failed_runs}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.workflowRuntime.metrics.failedHint")}</div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.workflowRuntime.metrics.queued")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{scopedWorkflowMetrics.queued_runs}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.workflowRuntime.metrics.queuedHint")}</div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.workflowRuntime.metrics.running")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{scopedWorkflowMetrics.running_runs}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.workflowRuntime.metrics.runningHint")}</div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.workflowRuntime.metrics.retries")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{scopedWorkflowMetrics.retry_runs}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.workflowRuntime.metrics.retriesHint")}</div>
                </div>
              </div>
              <div className="grid gap-4 border-t border-slate-100 p-6 xl:grid-cols-2">
                <div className="space-y-3">
                  <div className="text-sm font-semibold text-slate-950">{t("admin.workflowRuntime.pressureSignals")}</div>
                  <div className="grid gap-3 sm:grid-cols-3">
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                      <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.workflowRuntime.tenantsWithFailures")}</div>
                      <div className="mt-2 text-2xl font-semibold text-slate-950">{workflowSignals.tenantsWithFailures}</div>
                    </div>
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                      <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.workflowRuntime.tenantsWithQueuePressure")}</div>
                      <div className="mt-2 text-2xl font-semibold text-slate-950">{workflowSignals.tenantsWithQueuePressure}</div>
                    </div>
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                      <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.workflowRuntime.tenantsWithRetries")}</div>
                      <div className="mt-2 text-2xl font-semibold text-slate-950">{workflowSignals.tenantsWithRetries}</div>
                    </div>
                  </div>
                </div>

                <div className="space-y-3">
                  <div className="text-sm font-semibold text-slate-950">{t("admin.workflowRuntime.tenantBreakdown")}</div>
                  {tenantWorkflowActivity.slice(0, 4).map((item) => (
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4" key={item.tenant.id}>
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                          {t("admin.workflowRuntime.failedRunCount", { count: String(item.metrics.failed_runs) })}
                        </Badge>
                      </div>
                      <div className="mt-3 flex flex-wrap gap-2">
                        <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                          {t("admin.workflowRuntime.queuePressureCount", { count: String(item.queuePressure) })}
                        </Badge>
                        <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                          {t("admin.workflowRuntime.retryRunCount", { count: String(item.metrics.retry_runs) })}
                        </Badge>
                      </div>
                      <div className="mt-4 flex flex-wrap gap-2">
                        <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                          <Link href={item.failedHref}>{t("admin.workflowRuntime.openFailedLane")}</Link>
                        </Button>
                        <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                          <Link href={item.queueHref}>{t("admin.workflowRuntime.openQueueLane")}</Link>
                        </Button>
                        <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                          <Link href={item.workspaceWorkflowHref}>{t("admin.workflowRuntime.openWorkflowSurface")}</Link>
                        </Button>
                      </div>
                    </div>
                  ))}
            ...
            <ConsoleSurface>
              <ConsoleSurfaceHeader
                action={
                  <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                    <Link href={documentSignals.highestPressureTenant?.documentsHref ?? buildAdminWorkspaceHref("overview", { view: "documents", tenantId: selectedTenantId === "all" ? null : selectedTenantId, workspaceId: scopedPrimaryWorkspace?.id ?? null, knowledgeBaseId: scopedPrimaryKnowledgeBase?.id ?? null })}>
              ...
              <div className="grid gap-4 p-6 md:grid-cols-2 xl:grid-cols-4">
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.documentRuntime.metrics.failed")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{scopedDocumentMetrics.failed_documents}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.documentRuntime.metrics.failedHint")}</div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.documentRuntime.metrics.active")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{scopedDocumentMetrics.active_documents}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.documentRuntime.metrics.activeHint")}</div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.documentRuntime.metrics.completed")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{scopedDocumentMetrics.completed_documents}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.documentRuntime.metrics.completedHint")}</div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.documentRuntime.metrics.total")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{scopedDocumentMetrics.total_documents}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.documentRuntime.metrics.totalHint")}</div>
                </div>
              </div>
              <div className="grid gap-4 border-t border-slate-100 p-6 xl:grid-cols-2">
                <div className="space-y-3">
                  <div className="text-sm font-semibold text-slate-950">{t("admin.documentRuntime.pressureSignals")}</div>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                      <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.documentRuntime.tenantsWithFailures")}</div>
                      <div className="mt-2 text-2xl font-semibold text-slate-950">{documentSignals.tenantsWithFailedDocuments}</div>
                    </div>
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                      <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.documentRuntime.tenantsWithActiveIntake")}</div>
                      <div className="mt-2 text-2xl font-semibold text-slate-950">{documentSignals.tenantsWithActiveIntake}</div>
                    </div>
                  </div>
                </div>

                <div className="space-y-3">
                  <div className="text-sm font-semibold text-slate-950">{t("admin.documentRuntime.tenantBreakdown")}</div>
                  {tenantDocumentActivity.slice(0, 4).map((item) => (
                    <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4" key={item.tenant.id}>
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                          {t("admin.documentRuntime.failedDocumentCount", { count: String(item.metrics.failed_documents) })}
                        </Badge>
                      </div>
                      <div className="mt-3 flex flex-wrap gap-2">
                        <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                          {t("admin.documentRuntime.activeDocumentCount", { count: String(item.metrics.active_documents) })}
                        </Badge>
                        <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                          {t("admin.documentRuntime.knowledgeBaseCount", { count: String(item.knowledgeBaseCount) })}
                        </Badge>
                      </div>
                      <div className="mt-4 flex flex-wrap gap-2">
                        <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                          <Link href={item.failedDocumentsHref}>{t("admin.documentRuntime.openFailedDocuments")}</Link>
                        </Button>
                        <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                          <Link href={item.documentsHref}>{t("admin.documentRuntime.openDocuments")}</Link>
                        </Button>
                        <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                          <Link href={item.operationsHref}>{t("admin.documentRuntime.openOperations")}</Link>
                        </Button>
                      </div>
                    </div>
                  ))}
            ...
            <ConsoleSurface>
              ...
              <div className="space-y-3 px-6 py-5">
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.chatScope.conversations")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{scopedConversationMetrics.total_conversations}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.chatScope.activeConversationThreads", { count: String(scopedConversationMetrics.active_conversations) })}</div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.chatScope.messages")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{scopedConversationMetrics.total_messages}</div>
                  <div className="mt-1 text-sm text-slate-500">
                    {scopedConversationMetrics.latest_activity_at
                      ? t("admin.chatScope.latestActivity", { value: formatTimestamp(scopedConversationMetrics.latest_activity_at) })
                      : t("admin.chatScope.noChatActivity")}
                  </div>
                </div>
                <Button asChild className="w-full bg-white" type="button" variant="outline">
                  <Link href={chatScopeHref}>{t("admin.chatScope.openScopedChatWorkspace")}</Link>
                </Button>
              </div>
            </ConsoleSurface>
            ) : null}
            ...
            <ConsoleSurface>
              ...
              <div className="space-y-3 px-6 py-5">
                {tenantChatActivity.slice(0, 6).map((item) => (
                  <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4" key={item.tenant.id}>
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                        {t("admin.tenantChat.messages", { count: String(item.metrics.total_messages) })}
                      </Badge>
                    </div>
                    <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
                      <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                        {t("admin.tenantChat.conversations", { count: String(item.metrics.total_conversations) })}
                      </Badge>
                      <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                        {t("admin.tenantChat.active", { count: String(item.metrics.active_conversations) })}
                      </Badge>
                      <Badge className="border-slate-200 bg-white text-slate-700" variant="outline">
                        {t("admin.tenantChat.workspaces", { count: String(item.workspaceCount) })}
                      </Badge>
                    </div>
                    <div className="mt-3 text-sm text-slate-500">
                      {item.metrics.latest_activity_at
                        ? t("admin.tenantChat.latestActivity", { value: formatTimestamp(item.metrics.latest_activity_at) })
                        : t("admin.tenantChat.noChatActivity")}
                    </div>
                    <div className="mt-4">
                      <div className="flex flex-wrap gap-2">
                        <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                          <Link href={item.openHref}>{t("admin.tenantChat.openTenantChat")}</Link>
                        </Button>
                        <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                          <Link href={item.documentsHref}>{t("shell.nav.documents")}</Link>
                        </Button>
                        <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                          <Link href={item.workflowsHref}>{t("shell.userMenu.operations")}</Link>
                        </Button>
                        <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                          <Link href={item.failedDocumentsHref}>{t("admin.tenantChat.failedDocs")}</Link>
                        </Button>
                        <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                          <Link href={item.failedWorkflowsHref}>{t("admin.tenantChat.failedWorkflows")}</Link>
                        </Button>
                        <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                          <Link href={item.governanceHref}>{t("admin.tenantChat.governance")}</Link>
                        </Button>
                      </div>
                    </div>
                  </div>
                ...
                {!isLoading && tenantChatActivity.length === 0 ? (
                  <div className="text-sm text-slate-500">{t("admin.tenantChat.noTenantActivity")}</div>
                ) : null}
              </div>
            </ConsoleSurface>
            ) : null}
            ...
            <ConsoleSurface>
              ...
              <div className="space-y-3 px-6 py-5">
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.chatSignals.mostActiveTenant")}</div>
                  <div className="mt-2 text-base font-semibold text-slate-900">
                    {chatSignals.activeTenant?.tenant.name ?? t("admin.chatSignals.noTenantActivity")}
                  </div>
                  <div className="mt-1 text-sm text-slate-500">
                    {chatSignals.activeTenant
                      ? t("admin.chatSignals.activeTenantDetail", {
                          conversationCount: String(chatSignals.activeTenant.metrics.total_conversations),
                          messageCount: String(chatSignals.activeTenant.metrics.total_messages)
                        })
                      : t("admin.chatSignals.noPersistedMessages")}
                  </div>
                  <div className="mt-3">
                    <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                      <Link href={chatSignals.activeTenant?.openHref ?? chatScopeHref}>{t("admin.chatSignals.openTenantChat")}</Link>
                    </Button>
                  </div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.chatSignals.staleChatTenants")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{chatSignals.staleTenantCount}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.chatSignals.staleChatDetail")}</div>
                  <div className="mt-3">
                    <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                      <Link href={staleChatScopeHref}>{t("admin.chatSignals.reviewChatScope")}</Link>
                    </Button>
                  </div>
                </div>
                <div className="rounded-[20px] border border-slate-100 bg-slate-50/70 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{t("admin.chatSignals.idleConversationScope")}</div>
                  <div className="mt-2 text-2xl font-semibold text-slate-950">{chatSignals.idleConversationTenantCount}</div>
                  <div className="mt-1 text-sm text-slate-500">{t("admin.chatSignals.idleConversationDetail")}</div>
                  <div className="mt-3">
                    <Button asChild className="bg-white" size="sm" type="button" variant="outline">
                      <Link href={idleConversationScopeHref}>{t("admin.chatSignals.inspectIdleScope")}</Link>
                    </Button>
                  </div>
                </div>
              </div>
        ...
        {managementPanel === "tenant-create" ? (
          ...
          >
            <div className="space-y-5">
              ...
              <AdminManagementField label={t("workspace.sidebar.modal.tenantSlug")}>
                <Input
                  onChange={(event) => setCreateTenantSlug(slugifyValue(event.target.value))}
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <Button
                  disabled={!createTenantName.trim() || !createTenantSlug.trim() || isCreatingResource}
                  onClick={handleCreateTenant}
                  type="button"
                >
                  {isCreatingResource ? t("workspace.sidebar.modal.creating") : t("workspace.sidebar.modal.createTenant")}
                </Button>
              </div>
            </div>
          </AdminManagementDialog>
          ...
          >
            <div className="space-y-5">
              ...
              <AdminManagementField label={t("admin.members.role")}>
                <Select
                  onValueChange={(value) => setCreateUserRole(value as "super_admin" | "operator" | "reviewer")}
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="super_admin">{t("auth.roles.superAdmin")}</SelectItem>
                    <SelectItem value="operator">{t("auth.roles.operator")}</SelectItem>
                    <SelectItem value="reviewer">{t("auth.roles.reviewer")}</SelectItem>
                  </SelectContent>
                </Select>
              </AdminManagementField>

              <AdminManagementField label={t("admin.members.initialTenant")}>
                <Select onValueChange={setCreateUserTenantId} value={createUserTenantId}>
                  <SelectTrigger>
                    <SelectValue placeholder={t("admin.members.initialTenant")} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="none">{t("admin.members.noInitialTenant")}</SelectItem>
                    {tenants.map((tenant) => (
                      <SelectItem key={tenant.id} value={tenant.id}>
                        {tenant.name}
                      </SelectItem>
              ...
              <AdminManagementField label={t("admin.members.initialMembershipStatus")}>
                <Select
                  onValueChange={(value) => setCreateUserMembershipStatus(value as "active" | "invited" | "suspended")}
                  value={createUserMembershipStatus}
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t("admin.members.initialMembershipStatus")} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="active">{t("admin.members.activeMembership")}</SelectItem>
                    <SelectItem value="invited">{t("admin.members.invitedMembership")}</SelectItem>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <Button
                  disabled={!createUserDisplayName.trim() || !createUserEmail.trim() || isCreatingResource}
                  onClick={handleCreateUser}
                  type="button"
                >
                  {isCreatingResource ? t("workspace.sidebar.modal.creating") : t("admin.actions.createMember")}
                </Button>
              </div>
            </div>
          </AdminManagementDialog>
        ...
        {managementPanel === "user-edit" ? (
          ...
          >
            <div className="space-y-5">
              ...
              <AdminManagementField label={t("admin.members.role")}>
                <Select
                  onValueChange={(value) => setEditUserRole(value as "super_admin" | "operator" | "reviewer")}
                  value={editUserRole}
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t("admin.members.role")} />
              ...
              <AdminManagementField label={t("admin.members.accountFilter")}>
                <Select onValueChange={(value) => setEditUserIsActive(value as "active" | "inactive")} value={editUserIsActive}>
                  <SelectTrigger>
                    <SelectValue placeholder={t("admin.members.accountFilter")} />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <Button
                  disabled={!editUserDisplayName.trim() || !editUserEmail.trim() || isUpdatingResource}
                  onClick={() => void handleUpdateUser()}
                  type="button"
                >
                  {isUpdatingResource ? t("workspace.sidebar.modal.saving") : t("admin.actions.saveMember")}
                </Button>
              </div>
            </div>
          </AdminManagementDialog>
        ...
        {managementPanel === "workspace-create" ? (
          ...
          >
            <div className="space-y-5">
              <AdminManagementField label={t("workspace.sidebar.sectionTenant")}>
                <Select onValueChange={setCreateWorkspaceTenantId} value={createWorkspaceTenantId}>
                  <SelectTrigger>
              ...
              <AdminManagementField label={t("workspace.sidebar.modal.workspaceName")}>
                <Input
                  autoFocus
                  onChange={(event) => {
                    const nextName = event.target.value;
              ...
              >
                <Input
                  onChange={(event) => setCreateWorkspaceSlug(slugifyValue(event.target.value))}
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <Button
                  disabled={
                    !createWorkspaceTenantId || !createWorkspaceName.trim() || !createWorkspaceSlug.trim() || isCreatingResource
                  }
                  onClick={handleCreateWorkspace}
                  type="button"
                >
                  {isCreatingResource ? t("workspace.sidebar.modal.creating") : t("workspace.sidebar.modal.createWorkspace")}
                </Button>
              </div>
            </div>
          </AdminManagementDialog>
        ...
        {managementPanel === "workspace-edit" ? (
          ...
          >
            <div className="space-y-5">
              <AdminManagementField label={t("workspace.sidebar.sectionTenant")}>
                <Select onValueChange={setEditWorkspaceTenantId} value={editWorkspaceTenantId}>
                  <SelectTrigger>
                    <SelectValue placeholder={t("workspace.sidebar.selectTenant")} />
                  </SelectTrigger>
                  <SelectContent>
              ...
              >
                <Input
                  onChange={(event) => setEditWorkspaceSlug(slugifyValue(event.target.value))}
                  placeholder={t("workspace.sidebar.modal.workspaceSlugPlaceholder")}
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <Button
                  disabled={!editWorkspaceTenantId || !editWorkspaceName.trim() || !editWorkspaceSlug.trim() || isUpdatingResource}
                  onClick={handleUpdateWorkspace}
                  type="button"
                >
                  {isUpdatingResource ? t("workspace.sidebar.modal.saving") : t("workspace.sidebar.modal.saveWorkspace")}
                </Button>
              </div>
            </div>
          </AdminManagementDialog>
        ...
        {managementPanel === "knowledge-base-create" ? (
          ...
          >
            <div className="space-y-5">
              <AdminManagementField label={t("workspace.sidebar.sectionWorkspace")}>
                <Select onValueChange={setCreateKnowledgeBaseWorkspaceId} value={createKnowledgeBaseWorkspaceId}>
                  <SelectTrigger>
                    <SelectValue placeholder={t("workspace.sidebar.selectWorkspace")} />
                  </SelectTrigger>
                  <SelectContent>
                    {(filteredWorkspaces.length > 0 ? filteredWorkspaces : workspaces).map((workspace) => (
              ...
              <AdminManagementField label={t("workspace.sidebar.modal.knowledgeBaseName")}>
                <Input
                  autoFocus
                  onChange={(event) => {
                    const nextName = event.target.value;
              ...
              >
                <Input
                  onChange={(event) => setCreateKnowledgeBaseSlug(slugifyValue(event.target.value))}
              ...
              >
                <Textarea
                  className="min-h-[112px] resize-y"
                  onChange={(event) => setCreateKnowledgeBaseDescription(event.target.value)}
              ...
              >
                <Select
                  onValueChange={(value) => setCreateKnowledgeBaseRetrievalProfileId(value === "none" ? "" : value)}
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <Button
                  ...
                  type="button"
                >
                  {isCreatingResource ? t("workspace.sidebar.modal.creating") : t("workspace.sidebar.modal.createKnowledgeBase")}
                </Button>
              </div>
            </div>
          </AdminManagementDialog>
        ...
        {managementPanel === "knowledge-base-edit" ? (
          ...
          >
            <div className="space-y-5">
              <AdminManagementField label={t("workspace.sidebar.sectionWorkspace")}>
                <Select onValueChange={setEditKnowledgeBaseWorkspaceId} value={editKnowledgeBaseWorkspaceId}>
                  <SelectTrigger>
                    <SelectValue placeholder={t("workspace.sidebar.selectWorkspace")} />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <AdminManagementField label={t("workspace.sidebar.modal.knowledgeBaseName")}>
                <Input
                  autoFocus
                  onChange={(event) => setEditKnowledgeBaseName(event.target.value)}
                  placeholder={t("workspace.sidebar.modal.knowledgeBaseNamePlaceholder")}
              ...
              >
                <Input
                  onChange={(event) => setEditKnowledgeBaseSlug(slugifyValue(event.target.value))}
                  placeholder={t("workspace.sidebar.modal.knowledgeBaseSlugPlaceholder")}
              ...
              >
                <Textarea
                  className="min-h-[112px] resize-y"
                  onChange={(event) => setEditKnowledgeBaseDescription(event.target.value)}
                  placeholder={t("workspace.sidebar.modal.knowledgeBaseDescriptionPlaceholder")}
              ...
              >
                <Select
                  onValueChange={(value) => setEditKnowledgeBaseRetrievalProfileId(value === "none" ? "" : value)}
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <Button
                  ...
                  type="button"
                >
                  {isUpdatingResource ? t("workspace.sidebar.modal.saving") : t("workspace.sidebar.modal.saveKnowledgeBase")}
                </Button>
              </div>
            </div>
          </AdminManagementDialog>
