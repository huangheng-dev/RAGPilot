      ...
      {paginatedResources.length ? (
        <div className="rounded-[20px] border border-slate-200 bg-white shadow-sm">
          {paginatedResources.map((item) => (
            <div
              className="flex flex-col gap-3 border-b border-slate-100 px-4 py-3.5 transition-colors last:border-b-0 hover:bg-slate-50/70 sm:px-5 lg:flex-row lg:items-center lg:justify-between"
              key={item.id}
            >
    ...
    >
      <DialogFormLayout>
        ...
        <div className="space-y-5">
          ...
          {kind==="tool_registration"?<DialogFormField label={t("admin.runtimeResources.fields.approval")}><Select value={bool("requires_admin_approval")?"required":"not_required"} onValueChange={(value)=>set("requires_admin_approval",value==="required")}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="required">{t("admin.runtimeResources.approvalRequired")}</SelectItem><SelectItem value="not_required">{t("admin.runtimeResources.approvalNotRequired")}</SelectItem></SelectContent></Select></DialogFormField>:null}
        </div>
      </DialogFormLayout>
    </FormDialog>
