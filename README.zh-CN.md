...
- URL 单页内容导入
- 内容解析、标准化、切片和向量化
- 向量持久化、Elasticsearch BM25 与 PostgreSQL 降级的混合检索
- 文档版本、重建、状态跟踪和失败恢复
...
- 智能体定义、知识范围、模型与工具绑定管理
- 基于 Temporal 的长流程执行、恢复、重试和取消
- 检索、MCP 工具和任务步骤之间的结构化运行交接
- 面向运营人员的谱系、结果审查、审计与运行诊断
...
- 检索配置、模型端点、限流、并发、重试与用量成本治理
- 原生工具、HTTP 工具和 Streamable HTTP MCP 连接器的发现、映射与治理
- 加密的运行时凭据、连接器与绑定治理，避免暴露原始密钥
...
- `Elasticsearch`：版本化 Chunk 投影、生命周期同步、范围回填、精确重建校验、读写 Alias 原子切换、租户安全 BM25 检索、PostgreSQL 降级与投影诊断已落地
- `OpenTelemetry`：API、Worker 与智能体运行链路的 Trace、指标与日志关联，配合 Collector、Prometheus 与 Tempo 进行运行诊断
...
- `LlamaIndex` 检索试点通道
- `LangGraph` 工作流恢复试点通道
- 受治理的 Streamable HTTP `MCP` 客户端链路，支持连接器发现、工具映射和智能体调用

这些集成都位于明确的运行时选择、配置和治理边界之后；依赖已安装不等于已经达到生产成熟度。独立 MCP 服务包仍是预留的对外服务边界。
...
-> Elasticsearch（生命周期投影、原子重建、BM25 召回与诊断已启用）
```
...
对公网开放前，应配置可信来源、替换所有模板 Secret、创建认证身份、验证模型端点连通性、执行数据库迁移，并完成备份恢复和可靠性演练。
