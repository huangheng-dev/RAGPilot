...
RAGPilot 是一个面向检索增强应用运营团队的开源 AI 知识运营平台，在同一系统中提供受治理的知识接入、权限感知混合检索、带引用的 Chat、受约束 Agent、持久化工作流和运行管理。

Web 控制台、API、Worker、持久化模型、运行策略、部署资产和发布验证在一个版本化 monorepo 中统一维护。RAGPilot 的设计重点是明确范围、可恢复状态和可审查证据。
...
- `Admin`：租户、工作空间、知识库、成员、访问、模型、工具、连接器和检索治理
...
- 位于检索引擎契约后的 `LlamaIndex` 检索对比
- 位于 Temporal 持久化 Agent 工作流内部的 `LangGraph` 有界图执行

试点依赖不是平台强制基础，依赖存在也不代表已经提升为默认运行路径。

## 版本范围

- 内置 Web 连接器按数据源同步一个公开页面，不是全站爬虫
- OCR 支持扫描 PDF 和列出的独立图片格式，但不承诺完整复杂版面或表格重建
- 本地密码认证可以运行；部署环境的 OIDC/SAML 回调和运维闭环属于环境相关演进工作
- Kubernetes 清单是面向生产的交付基线，不代表未经配置的仓库可以直接开放公网
- Provider-neutral Rerank 和更广泛框架通道必须通过评估证据才能提升为默认能力

当前实现细节见 [Project Snapshot](./docs/product/project-snapshot.md)，长期架构规则见 [Project Blueprint](./docs/product/project-blueprint.md)，工程演进优先级见 [Roadmap](./docs/planning/roadmap.md)。
