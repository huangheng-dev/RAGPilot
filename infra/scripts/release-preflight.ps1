...
Write-Host ""
Write-Host "1/13 Auditing public documentation..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "2/13 Auditing public markdown links..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "3/13 Linting web..." -ForegroundColor Cyan
& npm run web:lint
if ($LASTEXITCODE -ne 0) {
...
Write-Host ""
Write-Host "4/13 Building web..." -ForegroundColor Cyan
& npm run web:check
if ($LASTEXITCODE -ne 0) {
...
Write-Host ""
Write-Host "5/13 Auditing production Node dependencies..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "6/13 Running API tests..." -ForegroundColor Cyan
& npm run api:test
if ($LASTEXITCODE -ne 0) {
...
Write-Host ""
Write-Host "7/13 Running Worker tests..." -ForegroundColor Cyan
& npm run worker:test
if ($LASTEXITCODE -ne 0) {
...
Write-Host ""
Write-Host "8/13 Building and testing the MCP server..." -ForegroundColor Cyan
& npm run mcp:build
if ($LASTEXITCODE -ne 0) {
...
Write-Host ""
Write-Host "9/13 Running isolated authenticated browser E2E tests..." -ForegroundColor Cyan
& npm run e2e:test
if ($LASTEXITCODE -ne 0) {
...
Write-Host ""
Write-Host "10/13 Auditing public candidate files..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "11/13 Auditing synchronized dependency locks..." -ForegroundColor Cyan
& $powerShellHost -NoProfile -ExecutionPolicy Bypass -File "infra/scripts/dependency-lock-audit.ps1"
if ($LASTEXITCODE -ne 0) {
  throw "Dependency lock audit failed."
}

Write-Host ""
Write-Host "12/13 Auditing production delivery assets..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "13/13 Running secret scan..." -ForegroundColor Cyan
