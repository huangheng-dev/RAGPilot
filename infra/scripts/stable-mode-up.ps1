...
$stableApiOverrides = @{
  ...
  RETRIEVAL_ENGINE                   = if ($rootEnvironment.ContainsKey("RETRIEVAL_ENGINE")) { $rootEnvironment["RETRIEVAL_ENGINE"] } else { "native" }
  AGENT_RUNTIME_ENGINE               = if ($rootEnvironment.ContainsKey("AGENT_RUNTIME_ENGINE")) { $rootEnvironment["AGENT_RUNTIME_ENGINE"] } else { "native" }
